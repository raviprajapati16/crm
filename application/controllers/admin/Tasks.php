...
class Tasks extends AdminController
{
    ...
    public function list_tasks($id = '')
    {
        ...
        // 🔹 Handle toggle requests (?switch_kanban=true/false)
        if ($this->input->get('switch_kanban')) {
            $set = $this->input->get('switch_kanban') == 'true' ? 'true' : 'false';
            $this->session->set_userdata('tasks_kanban_view', $set);
            redirect(admin_url('tasks'));
        }

        // 🔹 Default to Kanban view if not set yet
        if ($this->session->userdata('tasks_kanban_view') === null) {
            $this->session->set_userdata('tasks_kanban_view', 'true');
        }

        // 🔹 Get current Kanban state
        $isKanban = $this->session->userdata('tasks_kanban_view') == 'true';

        // 🔹 URL parameter for filters (optional)
        $data['custom_view'] = $this->input->get('custom_view') ?: '';
        $data['taskid']      = $id;

        // 🔹 If Kanban mode is active
        if ($isKanban) {
            $data['switch_kanban'] = true;
            $data['bodyclass']     = 'tasks-page kan-ban-body';
        } else {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    if ($data['rel_type'] == "lead") {
                        leadLastContactAtUpdate($data['rel_id']);
                    }
                    $success       = true;
                    $_id           = $id;
