...
class Payments extends AdminController
{
    ...
    public function payment($id = '')
    {
        ...
        if ($payment->invoiceid == "0") {
            $template_name = 'proposal_advance_payment_recorded_to_customer';
            $data = prepare_mail_preview_data($template_name, null);
        } else {
        ...
        if ($payment->invoiceid == "0") {
            $data['title'] = _l('payment_receipt') . ' - ' . format_proposal_number($data['payment']->proposal_id);
        } else {
    ...
    public function send_to_email($id)
    {
            ...
            if (is_array($sent_to) && count($sent_to) > 0) {
                foreach ($sent_to as $contact_id) {
                    if ($contact_id != '') {
                        ...
                        if ($payment->invoiceid != "0") {
                            $template = mail_template('invoice_payment_recorded_to_customer', (array) $contact, $payment->invoice_data, false, $payment->paymentid, $staff->email);
                        } else {
                            $this->load->model('leads_model');
                            $this->load->model('proposals_model');
                            $proposal_data = $this->proposals_model->get($payment->proposal_id);
                            $proposal_data->paymentid = $payment->paymentid;
                            $template = mail_template('proposal_advance_payment_recorded_to_customer', $contact->email, $proposal_data, $staff->email);
                        }
        ...
        } else if (isset($post_data['lead_sent_to'])) {
            $this->load->model('leads_model');
            $this->load->model('proposals_model');
            $sent_to = $post_data['lead_sent_to'];
            if (is_array($sent_to) && count($sent_to) > 0) {
                foreach ($sent_to as $contact_id) {
                    if ($contact_id != '') {
