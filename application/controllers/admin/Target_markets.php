...
class Target_markets extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/target_markets/manage', $data);
    }
    ...
    public function table()
    {
        ...
        $this->app->get_table_data('target_markets');
    }
    ...
    public function all_contacts()
    {
        ...
        $this->load->view('admin/target_markets/all_contacts', $data);
    }
    ...
    {
        if (!has_permission('customers', '', 'view')) {
            if ($id != '' && !is_customer_admin_new($id)) {
                access_denied('customers');
            }
        }
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if ($save_and_add_contact == false) {
                        ...
                        redirect(admin_url('target_markets/client/' . $id . '?group=contacts&new_contact=true'));
                    }
                }
            } else {
        ...
        if ($group != 'contacts' && $contact_id = $this->input->get('contactid')) {
            redirect(admin_url('target_markets/client/' . $id . '?group=contacts&contactid=' . $contact_id));
        }
        ...
        if ($id == '') {
            ...
            $tabs_to_hide = [
                ...
                'tickets',
                'customer_media',
                'customer_assigned_leads'
            ];
            foreach ($tabs_to_hide as $tab_to_hide) {
                if (isset($data['customer_tabs'][$tab_to_hide])) {
                    unset($data['customer_tabs'][$tab_to_hide]);
                }
            }
        ...
        $this->load->view('admin/target_markets/target_market', $data);
    }
    ...
    public function check_duplicate_customer_name()
    {
        if (has_permission('customers', '', 'create')) {
            ...
            $response    = [
                'exists'  => (bool) total_rows(db_prefix() . 'clients', ['company' => $companyName, 'deleted_at' => NULL]) > 0,
    ...
    {
        if (!has_permission('customers', '', 'view')) {
            if (!is_customer_admin_new($customer_id)) {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                echo json_encode([
                    'success'             => $success,
                    'message'             => $message,
                    'has_primary_contact' => (total_rows(db_prefix() . 'contacts', ['userid' => $customer_id, 'is_primary' => 1, 'deleted_at' => NULL]) > 0 ? true : false),
                    'is_individual'       => is_empty_customer_company($customer_id) && total_rows(db_prefix() . 'contacts', ['userid' => $customer_id, 'deleted_at' => NULL]) == 1,
                ]);
                die;
            }
            if (!has_permission('customers', '', 'edit')) {
        ...
        $data['title']                = $title;
        $this->load->view('admin/target_markets/modals/contact', $data);
    }
    ...
    public function assign_admins($id)
    {
        ...
        redirect(admin_url('target_markets/client/' . $id . '?tab=customer_admins'));
    }
    ...
    public function delete_customer_admin($customer_id, $staff_id)
    {
        ...
        $this->db->update(db_prefix() . 'customer_admins', ['deleted_at' => date('Y-m-d H:i:s'), 'deleted_by' => get_staff_full_name()]);
        redirect(admin_url('target_markets/client/' . $customer_id) . '?tab=customer_admins');
    }
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        redirect(admin_url('target_markets/client/' . $customer_id . '?group=contacts'));
    }
    ...
    public function import()
    {
        ...
        $this->load->view('admin/target_markets/import', $data);
    }
    ...
    public function groups()
    {
        ...
        $this->load->view('admin/target_markets/groups_manage', $data);
    }
    ...
    public function delete_group($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('target_markets/groups'));
        }
    ...
    public function statement_pdf()
    {
        ...
        if (!has_permission('invoices', '', 'view') && !has_permission('payments', '', 'view')) {
            set_alert('danger', _l('access_denied'));
            redirect(admin_url('target_markets/client/' . $customer_id));
        }
        ...
        try {
            $pdf = statement_mpdf($data['statement']);
        } catch (Exception $e) {
    ...
    public function send_statement()
    {
        ...
        redirect(admin_url('target_markets/client/' . $customer_id . '?group=statement'));
    }
    ...
    public function statement()
    {
        ...
        $viewData['html'] = $this->load->view('admin/target_markets/groups/_statement', $data, true);
