<?php
defined('BASEPATH') OR exit('No direct script access allowed');

/**
 * Invoice_map Controller
 *
 * Hierarchical Invoice Choropleth Map
 *   World → Country (states/provinces) → State (cities) → City invoice list
 *
 * GeoJSON: local-first boundaries via geoBoundaries (Geojson_dataset).
 */
class Invoice_map extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        if (!has_permission('reports', '', 'view')) {
            access_denied('Invoice Map');
        }
        $this->load->model('invoice_map_model');
        $this->load->model('invoices_model');
    }

    // =========================================================================
    // PAGE
    // =========================================================================
    public function index()
    {
        $this->load->model('currencies_model');
        $this->load->model('proposals_model');
        $data['title'] = 'Invoice Map View';
        $data['invoices_branches_gst'] = $this->proposals_model->get_proposals_branches_gst();

        if (is_using_multiple_currencies()) {
            $data['currencies'] = $this->currencies_model->get();
        }

        $data['geojson_version'] = $this->invoice_map_model->get_geojson_version_token();

        $this->load->view('admin/invoice_map/index', $data);
    }

    // =========================================================================
    // AJAX: choropleth heat-map data
    // POST: level (world|country|state), country (ISO2), state, + date/status filters
    // =========================================================================
    public function map_data()
    {
        if (!$this->input->is_ajax_request()) { show_404(); }

        $level   = $this->input->post('level')   ?: 'world';
        $iso2    = $this->input->post('country');
        $state   = $this->input->post('state');
        $filters = $this->_build_filters();

        // Sanitise level
        if (!in_array($level, ['world', 'country', 'state'])) {
            $level = 'world';
        }
        if ($iso2)  $iso2  = strtoupper(preg_replace('/[^a-zA-Z]/', '', $iso2));
        if ($state) $state = $this->_canonicalize_state_name($state);

        switch ($level) {
            case 'country':
                if (!$iso2) { $this->_json_error('Country code required'); }
                $data = $this->invoice_map_model->get_country_data($iso2, $filters);
                break;

            case 'state':
                if (!$iso2 || !$state) { $this->_json_error('Country and state required'); }
                $data = $this->invoice_map_model->get_state_data($iso2, $state, $filters);
                break;

            default:
                $data  = $this->invoice_map_model->get_world_data($filters);
                $level = 'world';
        }

        $this->load->model('currencies_model');
        $base_currency = $this->currencies_model->get_base_currency()->name;

        foreach ($data as &$row) {
            $currency_name = !empty($row['currency_name']) ? $row['currency_name'] : $base_currency;
            $row['total_formatted'] = app_format_money($row['total_amount'], $currency_name);
        }

        echo json_encode([
            'success'    => true,
            'level'      => $level,
            'iso2'       => $iso2,
            'state'      => $state,
            'data'       => $data,
            'breadcrumb' => $this->_build_breadcrumb($level, $iso2, $state),
        ]);
        die();
    }

    // =========================================================================
    // AJAX: city invoice list (paginated, 100/page)
    // =========================================================================
    public function city_invoices()
    {
        if (!$this->input->is_ajax_request()) { show_404(); }

        $iso2    = $this->input->post('country');
        $state   = $this->input->post('state');
        $city    = $this->input->post('city');
        $page    = max(0, (int) $this->input->post('page'));
        $filters = $this->_build_filters();

        if (!$iso2) {
            $this->_json_error('Country is required');
        }

        $iso2  = strtoupper(preg_replace('/[^a-zA-Z]/', '', $iso2));
        $state = $state ? $this->_canonicalize_state_name($state) : '';
        $city  = $city ? trim($city) : '';

        $result = $this->invoice_map_model->get_city_invoices(
            $iso2, $state, $city, $filters, $page
        );

        $this->load->model('currencies_model');
        $base_currency = $this->currencies_model->get_base_currency()->name;

        foreach ($result['invoices'] as &$inv) {
            $currency_name = !empty($inv['currency_name']) ? $inv['currency_name'] : $base_currency;
            $inv['total_formatted'] = app_format_money($inv['total'], $currency_name);
            $inv['date_formatted']  = _d($inv['date']);
            $inv['status_label']    = format_invoice_status($inv['status'], '', true);
            $inv['invoice_url']     = admin_url('invoices/list_invoices/' . $inv['id']);
        }

        echo json_encode([
            'success'         => true,
            'total_count'     => $result['count'],
            'total_amount'    => $result['total_amount'],
            'total_formatted' => app_format_money($result['total_amount'], $currency_name),
            'invoices'        => $result['invoices'],
            'page'            => $page,
            'has_more'        => count($result['invoices']) === 100,
        ]);
        die();
    }

    // =========================================================================
    // EXPORT: CSV Download
    // =========================================================================
    public function export_csv()
    {
        $level   = $this->input->post('level') ?: 'world';
        $iso2    = $this->input->post('country');
        $state   = $this->input->post('state');
        $city    = $this->input->post('city');
        $filters = $this->_build_filters();

        $invoices = $this->invoice_map_model->get_export_invoices($level, $iso2, $state, $city, $filters);

        $company_name = get_option('invoice_company_name') ?: get_option('companyname');
        $scope        = $this->_export_scope_label($level, $iso2, $state, $city);
        $filters_text = $this->_export_filters_summary($filters);

        $this->load->model('currencies_model');
        $base_currency      = $this->currencies_model->get_base_currency()->name;
        $totals_by_currency = $this->_export_totals_by_currency($invoices, $base_currency);
        $totals_by_status   = $this->_export_totals_by_status($invoices, $base_currency);

        header('Content-Type: application/vnd.ms-excel; charset=utf-8');
        header('Content-Disposition: attachment; filename=invoices_map_export_' . date('Ymd_His') . '.xls');

        $cols = 10;
        echo '<html xmlns:x="urn:schemas-microsoft-com:office:excel">';
        echo '<head><meta charset="utf-8">';
        echo '<style>table{border-collapse:collapse;font-family:Calibri,Arial,sans-serif;font-size:11pt;} .text{mso-number-format:"\\@";}</style>';
        echo '</head><body>';

        // Header
        echo '<table border="0" cellpadding="4"><tr><td colspan="' . $cols . '"><b>' . htmlspecialchars($company_name) . '</b></td></tr>';
        echo '<tr><td colspan="' . $cols . '"><font color="#28B8DA"><b>INVOICE MAP REPORT</b></font></td></tr>';
        echo '<tr><td colspan="' . $cols . '">Scope: ' . htmlspecialchars($scope) . '</td></tr>';
        if ($filters_text !== '') {
            echo '<tr><td colspan="' . $cols . '">' . htmlspecialchars($filters_text) . '</td></tr>';
        }
        echo '<tr><td colspan="' . $cols . '">Generated on ' . date('d M Y h:i A') . ' by ' . htmlspecialchars(get_staff_full_name(get_staff_user_id())) . '</td></tr>';
        echo '</table><br>';

        // Summary
        echo '<table border="1" cellpadding="5"><tr><td><b>Total Invoices</b></td><td>' . count($invoices) . '</td></tr>';

        $status_order = [
            Invoices_model::STATUS_OVERDUE,
            Invoices_model::STATUS_UNPAID,
            Invoices_model::STATUS_PARTIALLY,
            Invoices_model::STATUS_PAID,
            Invoices_model::STATUS_DRAFT,
            Invoices_model::STATUS_CANCELLED,
        ];
        foreach ($status_order as $status_id) {
            if (empty($totals_by_status[$status_id])) {
                continue;
            }
            $status_label = format_invoice_status($status_id, '', false);
            $status_color = $this->_export_status_color($status_id);
            $amount_text  = $this->_export_format_amounts($totals_by_status[$status_id], $base_currency);
            echo '<tr><td>' . htmlspecialchars($status_label) . ':</td>';
            echo '<td><font color="' . $status_color . '"><b>' . $amount_text . '</b></font></td></tr>';
        }

        foreach ($totals_by_currency as $c_name => $t_amount) {
            $label = count($totals_by_currency) > 1 ? 'Total (' . $c_name . ')' : 'Total Amount';
            echo '<tr><td><b>' . htmlspecialchars($label) . '</b></td><td>' . app_format_money($t_amount, $c_name) . '</td></tr>';
        }
        echo '</table><br>';

        // Data
        $headers = ['#', 'Invoice Number', 'Client', 'Date', 'Due Date', 'Total Amount', 'Status', 'Country', 'State', 'City'];
        echo '<table border="1" cellpadding="5"><tr>';
        foreach ($headers as $h) {
            echo '<th bgcolor="#E8E8E8"><b>' . $h . '</b></th>';
        }
        echo '</tr>';

        if (empty($invoices)) {
            echo '<tr><td colspan="' . $cols . '" align="center">No invoices found.</td></tr>';
        } else {
            $counter = 1;
            foreach ($invoices as $inv) {
                $status_label = strip_tags(format_invoice_status($inv['status'], '', false));
                $status_color = $this->_export_status_color($inv['status']);
                $currency     = !empty($inv['currency_name']) ? $inv['currency_name'] : $base_currency;

                echo '<tr>';
                echo '<td align="center">' . $counter . '</td>';
                echo '<td class="text">' . htmlspecialchars($inv['number']) . '</td>';
                echo '<td>' . htmlspecialchars($inv['client']) . '</td>';
                echo '<td align="center">' . _d($inv['date']) . '</td>';
                echo '<td align="center">' . _d($inv['duedate']) . '</td>';
                echo '<td align="right">' . app_format_money($inv['total'], $currency) . '</td>';
                echo '<td align="center"><font color="' . $status_color . '"><b>' . htmlspecialchars($status_label) . '</b></font></td>';
                echo '<td>' . htmlspecialchars($inv['country']) . '</td>';
                echo '<td>' . htmlspecialchars($inv['state']) . '</td>';
                echo '<td>' . htmlspecialchars($inv['city']) . '</td>';
                echo '</tr>';
                $counter++;
            }
        }

        echo '</table></body></html>';
        exit;
    }

    // =========================================================================
    // EXPORT: PDF Download
    // =========================================================================
    public function export_pdf()
    {
        $level   = $this->input->post('level') ?: 'world';
        $iso2    = $this->input->post('country');
        $state   = $this->input->post('state');
        $city    = $this->input->post('city');
        $filters = $this->_build_filters();

        $invoices = $this->invoice_map_model->get_export_invoices($level, $iso2, $state, $city, $filters);

        $company_name = get_option('invoice_company_name') ?: get_option('companyname');
        $scope        = $this->_export_scope_label($level, $iso2, $state, $city);
        $filters_text = $this->_export_filters_summary($filters);
        $generated_on = date('d M Y h:i A');
        $generated_by = get_staff_full_name(get_staff_user_id());

        $this->load->model('currencies_model');
        $base_currency      = $this->currencies_model->get_base_currency()->name;
        $totals_by_currency = $this->_export_totals_by_currency($invoices, $base_currency);
        $totals_by_status   = $this->_export_totals_by_status($invoices, $base_currency);

        $th_style    = 'background-color:#f8f9fa; color:#2d2d2d; font-weight:bold; text-align:center; border:1px solid #dcdcdc;';
        $label_style = 'font-weight:bold; background-color:#f4f5f7;';
        $col_count   = 10;

        $html = $this->_export_pdf_logo_html();

        $html .= '<h2 style="text-align:center; font-weight:bold; color:#323a45; margin:4px 0;">' . htmlspecialchars($company_name) . '</h2>';
        $html .= '<h3 style="text-align:center; font-weight:bold; color:#28b8da; letter-spacing:1px; margin:4px 0;">INVOICE MAP REPORT</h3>';
        $html .= '<p style="text-align:center; font-size:12px; color:#555; margin:6px 0;">Scope: ' . htmlspecialchars($scope) . '</p>';
        if ($filters_text !== '') {
            $html .= '<p style="text-align:center; font-size:10px; color:#555; margin:4px 0;">' . htmlspecialchars($filters_text) . '</p>';
        }
        $html .= '<p style="text-align:right; font-size:9px; color:#777; margin:8px 0;">Generated on ' . htmlspecialchars($generated_on) . ' by ' . htmlspecialchars($generated_by) . '</p>';

        $html .= '<table width="40%" border="1" cellpadding="6" cellspacing="0">';
        $html .= '<tr><td colspan="2" style="font-weight:bold; background-color:#f7f9fa;">Summary</td></tr>';
        $html .= '<tr><td width="60%" style="' . $label_style . '">Total Invoices:</td>';
        $html .= '<td width="40%">' . count($invoices) . '</td></tr>';

        $status_order = [
            Invoices_model::STATUS_OVERDUE,
            Invoices_model::STATUS_UNPAID,
            Invoices_model::STATUS_PARTIALLY,
            Invoices_model::STATUS_PAID,
            Invoices_model::STATUS_DRAFT,
            Invoices_model::STATUS_CANCELLED,
        ];
        foreach ($status_order as $status_id) {
            if (empty($totals_by_status[$status_id])) {
                continue;
            }
            $status_label = format_invoice_status($status_id, '', false);
            $status_color = $this->_export_status_color($status_id);
            $amount_text  = $this->_export_format_amounts($totals_by_status[$status_id], $base_currency);
            $html .= '<tr><td width="60%" style="padding-left:10px;">' . htmlspecialchars($status_label) . ':</td>';
            $html .= '<td width="40%" style="color:' . $status_color . '; font-weight:bold;">' . $amount_text . '</td></tr>';
        }

        foreach ($totals_by_currency as $c_name => $t_amount) {
            $amount_label = count($totals_by_currency) > 1
                ? 'Total Amount (' . htmlspecialchars($c_name) . '):'
                : 'Total Amount:';
            $html .= '<tr><td style="' . $label_style . '">' . $amount_label . '</td>';
            $html .= '<td>' . app_format_money($t_amount, $c_name) . '</td></tr>';
        }
        $html .= '</table>';
        $html .= $this->_export_pdf_spacer(12);
        $html .= '<hr style="border:none; border-top:2px solid #28b8da;">';
        $html .= $this->_export_pdf_spacer(10);

        $headers = ['#', 'Invoice Number', 'Client', 'Date', 'Due Date', 'Total Amount', 'Status', 'Country', 'State', 'City'];
        $widths  = ['5%', '12%', '18%', '10%', '10%', '10%', '10%', '8%', '8%', '9%'];

        $html .= '<table border="1" cellpadding="5" cellspacing="0" width="100%">';
        $html .= '<tr>';
        foreach ($headers as $index => $h) {
            $html .= '<th width="' . $widths[$index] . '" style="' . $th_style . '">' . $h . '</th>';
        }
        $html .= '</tr>';

        if (empty($invoices)) {
            $html .= '<tr><td colspan="' . $col_count . '" style="text-align:center; padding:16px; color:#777; font-style:italic; background-color:#fafafa;">No invoices found for the selected scope and filters.</td></tr>';
        } else {
            $counter = 1;
            foreach ($invoices as $inv) {
                $status_label = strip_tags(format_invoice_status($inv['status'], '', false));
                $status_color = $this->_export_status_color($inv['status']);
                $inv_currency = !empty($inv['currency_name']) ? $inv['currency_name'] : $base_currency;
                $row_bg       = ($counter % 2 === 0) ? '#fafafa' : '#ffffff';

                $html .= '<tr style="background-color:' . $row_bg . ';">';
                $html .= '<td style="text-align:center;">' . $counter . '</td>';
                $html .= '<td>' . htmlspecialchars($inv['number']) . '</td>';
                $html .= '<td>' . htmlspecialchars($inv['client']) . '</td>';
                $html .= '<td style="text-align:center;">' . _d($inv['date']) . '</td>';
                $html .= '<td style="text-align:center;">' . _d($inv['duedate']) . '</td>';
                $html .= '<td style="text-align:right;">' . app_format_money($inv['total'], $inv_currency) . '</td>';
                $html .= '<td style="text-align:center; color:' . $status_color . '; font-weight:bold;">' . htmlspecialchars($status_label) . '</td>';
                $html .= '<td>' . htmlspecialchars($inv['country']) . '</td>';
                $html .= '<td>' . htmlspecialchars($inv['state']) . '</td>';
                $html .= '<td>' . htmlspecialchars($inv['city']) . '</td>';
                $html .= '</tr>';

                $counter++;
            }
        }

        $html .= '</table>';

        try {
            $pdf = new TCPDF('L', 'mm', 'A4', true, 'UTF-8', false);
            $pdf->SetCreator(PDF_CREATOR);
            $pdf->SetAuthor($company_name);
            $pdf->SetTitle('Invoice Map Report');
            $pdf->SetHeaderData('', 0, '', '', array(0,0,0), array(255,255,255));
            $pdf->setPrintHeader(false);
            $pdf->setPrintFooter(false);
            $pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
            $pdf->SetMargins(10, 10, 10);
            $pdf->SetAutoPageBreak(TRUE, 15);
            $pdf->setImageScale(PDF_IMAGE_SCALE_RATIO);
            $pdf->setLanguageArray(['a_meta_charset' => 'UTF-8']);
            $pdf->SetFont($this->_export_pdf_font_name(), '', $this->_export_pdf_font_size());
            $pdf->AddPage();
            $pdf->writeHTML($html, true, false, true, false, '');
            $pdf->Output('invoices_map_export_' . date('Ymd_His') . '.pdf', 'D');
        } catch (\Throwable $e) {
            log_message('error', 'Invoice Map PDF export failed: ' . $e->getMessage());
            show_error('Could not generate PDF: ' . $e->getMessage());
        }
    }

    // =========================================================================
    // GEOJSON endpoint — dynamic for any country/state in the world
    //
    // GET /admin/invoice_map/geojson?level=world
    //     /admin/invoice_map/geojson?level=country&iso2=IN
    //     /admin/invoice_map/geojson?level=state&iso2=IN&state=Gujarat
    // =========================================================================
    public function geojson()
    {
        $level = $this->input->get('level') ?: 'world';
        $iso2  = $this->input->get('iso2');
        $state = $this->input->get('state');
        $stateIso = $this->input->get('state_iso');

        // Sanitise
        $level = in_array($level, ['world', 'country', 'state']) ? $level : 'world';
        if ($iso2)  $iso2  = strtoupper(preg_replace('/[^a-zA-Z]/', '', $iso2));
        if ($stateIso) {
            $stateIso = strtoupper(preg_replace('/[^a-zA-Z0-9\-]/', '', $stateIso));
            $stateIso = substr($stateIso, 0, 16);
        }
        if ($state) {
            $state = html_entity_decode($state, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $state = strip_tags($state);
            $state = preg_replace('/\s*&\s*/', ' and ', $state);
            $state = preg_replace('/\s+/', ' ', trim($state));
            $state = substr($state, 0, 100);
        }

        $geojson = $this->invoice_map_model->get_geojson($level, $iso2, $state, $stateIso ?: null);

        if ($geojson === false) {
            $this->output->set_status_header(404);
            header('Content-Type: application/json');
            echo json_encode(['error' => 'GeoJSON not available for this region']);
            die();
        }

        $version = $this->invoice_map_model->get_geojson_version_token();
        header('Content-Type: application/json; charset=utf-8');
        header('Cache-Control: public, max-age=86400, must-revalidate');
        header('ETag: "' . $version . '"');
        header('Vary: Accept-Encoding');
        echo $geojson;
        die();
    }

    // =========================================================================
    // ADMIN: refresh local GeoJSON cache (geoBoundaries)
    // GET /admin/invoice_map/refresh_geojson?force=1
    // =========================================================================
    public function refresh_geojson()
    {
        if (!is_admin()) {
            access_denied('Invoice Map');
        }

        $force = (bool) $this->input->get('force');
        $iso2  = $this->input->get('country');

        $options = [
            'world'     => true,
            'countries' => $iso2 ? [strtoupper($iso2)] : 'active',
            'force'     => $force,
        ];

        $result = $this->invoice_map_model->refresh_geojson_dataset($options);

        header('Content-Type: application/json');
        echo json_encode([
            'success' => true,
            'result'  => $result,
            'version' => $this->invoice_map_model->get_geojson_version_token(),
        ]);
        die();
    }

    // =========================================================================
    // GEOCODE: on-demand geocoding for a single city
    // =========================================================================
    public function geocode_city()
    {
        if (!$this->input->is_ajax_request()) { show_404(); }

        $city  = $this->input->post('city');
        $state = $this->input->post('state');
        $iso2  = $this->input->post('iso2');

        if (!$city || !$iso2) {
            echo json_encode(['success' => false]);
            die();
        }

        $coords = $this->invoice_map_model->geocode_city($city, $state, $iso2);

        echo json_encode([
            'success'   => (bool) $coords,
            'latitude'  => $coords['latitude']  ?? null,
            'longitude' => $coords['longitude'] ?? null,
        ]);
        die();
    }

    // =========================================================================
    // PRIVATE HELPERS
    // =========================================================================
    /**
     * TCPDF cannot load local paths via <img src="..."> — embed as base64 with @ prefix.
     * Prefer company_logo (color) over company_logo_dark for PDF reports.
     */
    /**
     * TCPDF ignores CSS margin on tables — use an explicit height spacer row.
     */
    private function _export_pdf_spacer($height_px = 12)
    {
        $height_px = max(1, (int) $height_px);

        return '<table width="100%" border="0" cellpadding="0" cellspacing="0">'
            . '<tr><td height="' . $height_px . '" style="font-size:1px; line-height:' . $height_px . 'px;">&nbsp;</td></tr>'
            . '</table>';
    }

    private function _export_pdf_font_name()
    {
        $this->load->helper('pdf');
        $font = get_option('pdf_font');

        if ($font !== '' && in_array($font, get_pdf_fonts_list())) {
            return $font;
        }

        return 'dejavusans';
    }

    private function _export_pdf_font_size()
    {
        $size = get_option('pdf_font_size');

        return ($size !== '' && is_numeric($size)) ? (int) $size : 9;
    }

    private function _export_pdf_logo_html()
    {
        $width = (int) (get_option('pdf_logo_width') ?: 120);
        $path  = $this->_export_pdf_logo_path();

        if ($path === '' || !is_readable($path)) {
            return '';
        }

        $filesize = @filesize($path);
        if ($filesize === false || $filesize > 2 * 1024 * 1024) {
            return '';
        }

        $imageData = base64_encode(file_get_contents($path));
        if ($imageData === '') {
            return '';
        }

        return '<div style="text-align:center; margin-bottom:8px;">'
            . '<img src="@' . $imageData . '" width="' . $width . '" />'
            . '</div>';
    }

    private function _export_pdf_logo_path()
    {
        $company_dir = get_upload_path_by_type('company');

        $logo = get_option('company_logo');
        if ($logo !== '' && file_exists($company_dir . $logo)) {
            return $company_dir . $logo;
        }

        $custom = get_option('custom_pdf_logo_image_url');
        if ($custom !== '') {
            if (file_exists($custom)) {
                return $custom;
            }
            $relative = FCPATH . ltrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $custom), DIRECTORY_SEPARATOR);
            if (file_exists($relative)) {
                return $relative;
            }
        }

        $logo_dark = get_option('company_logo_dark');
        if ($logo_dark !== '' && file_exists($company_dir . $logo_dark)) {
            return $company_dir . $logo_dark;
        }

        return '';
    }

    private function _export_scope_label($level, $iso2, $state, $city)
    {
        $breadcrumb = $this->_build_breadcrumb($level, $iso2, $state);
        $scope      = implode(' › ', array_column($breadcrumb, 'label'));

        if (!empty($city)) {
            $scope .= ' › ' . $city;
        }

        return $scope;
    }

    private function _export_filters_summary($filters)
    {
        $parts = [];

        if (!empty($filters['date_from']) && !empty($filters['date_to'])) {
            $parts[] = 'Period: ' . _d($filters['date_from']) . ' to ' . _d($filters['date_to']);
        }

        if (!empty($filters['status'])) {
            $statuses = is_array($filters['status']) ? $filters['status'] : [$filters['status']];
            $labels   = [];
            foreach ($statuses as $status) {
                $labels[] = format_invoice_status((int) $status, '', false);
            }
            $parts[] = 'Status: ' . implode(', ', $labels);
        }

        if (!empty($filters['currency'])) {
            $parts[] = 'Currency: ' . $filters['currency'];
        }

        if (!empty($filters['gst_numbers'])) {
            $gst = is_array($filters['gst_numbers']) ? $filters['gst_numbers'] : [$filters['gst_numbers']];
            $parts[] = 'GST: ' . implode(', ', $gst);
        }

        return implode(' | ', $parts);
    }

    private function _export_totals_by_currency($invoices, $base_currency)
    {
        $totals = [];

        foreach ($invoices as $inv) {
            $currency = !empty($inv['currency_name']) ? $inv['currency_name'] : $base_currency;
            if (!isset($totals[$currency])) {
                $totals[$currency] = 0;
            }
            $totals[$currency] += $inv['total'];
        }

        return $totals;
    }

    private function _export_totals_by_status($invoices, $base_currency)
    {
        $totals = [];

        foreach ($invoices as $inv) {
            $status   = (int) $inv['status'];
            $currency = !empty($inv['currency_name']) ? $inv['currency_name'] : $base_currency;

            if (!isset($totals[$status])) {
                $totals[$status] = [];
            }
            if (!isset($totals[$status][$currency])) {
                $totals[$status][$currency] = 0;
            }
            $totals[$status][$currency] += (float) $inv['total'];
        }

        return $totals;
    }

    private function _export_format_amounts(array $amounts_by_currency, $base_currency)
    {
        $parts = [];

        foreach ($amounts_by_currency as $currency => $amount) {
            $parts[] = app_format_money($amount, $currency);
        }

        return implode(' + ', $parts) ?: app_format_money(0, $base_currency);
    }

    private function _export_status_color($status)
    {
        $colors = [
            Invoices_model::STATUS_UNPAID    => '#FC2D42',
            Invoices_model::STATUS_PAID      => '#84C529',
            Invoices_model::STATUS_PARTIALLY => '#FF6F00',
            Invoices_model::STATUS_OVERDUE   => '#FF6F00',
            Invoices_model::STATUS_DRAFT     => '#777777',
            Invoices_model::STATUS_CANCELLED => '#777777',
        ];

        return $colors[(int) $status] ?? '#333333';
    }

    private function _canonicalize_state_name($state)
    {
        if ($state === null || $state === '') {
            return null;
        }

        $state = html_entity_decode((string) $state, ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $state = strip_tags($state);
        $state = preg_replace('/\s*&\s*/', ' and ', $state);
        $state = preg_replace('/_+/', ' ', $state);
        $state = preg_replace('/\s+/', ' ', trim($state));

        return $state !== '' ? substr($state, 0, 100) : null;
    }

    private function _build_filters()
    {
        return [
            'date_from'   => $this->input->post('date_from'),
            'date_to'     => $this->input->post('date_to'),
            'status'      => $this->input->post('status') ?: [],
            'currency'    => $this->input->post('currency'),
            'gst_numbers' => $this->input->post('gst_numbers') ?: [],
        ];
    }

    private function _build_breadcrumb($level, $iso2 = null, $state = null)
    {
        $bc = [['label' => 'World', 'level' => 'world', 'iso2' => null, 'state' => null]];

        if ($iso2) {
            $this->db->select('long_name')->where('iso2', $iso2);
            $row   = $this->db->get(db_prefix() . 'countries')->row();
            $bc[]  = [
                'label' => $row ? $row->long_name : $iso2,
                'level' => 'country',
                'iso2'  => $iso2,
                'state' => null,
            ];
        }

        if ($state) {
            $bc[] = ['label' => $state, 'level' => 'state', 'iso2' => $iso2, 'state' => $state];
        }

        return $bc;
    }

    private function _json_error($message, $code = 400)
    {
        $this->output->set_status_header($code);
        echo json_encode(['success' => false, 'message' => $message]);
        die();
    }
}
