...
class Emails extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['proposalterms'] = $this->emails_model->get_with_order([
            'type' => 'proposalterms',
            'language' => 'english',
        ]);
        ...
        $data['vendors'] = $this->emails_model->get([
            'type' => 'vendors',
            'language' => 'english',
        ]);

        $data['goals'] = $this->emails_model->get([
            'type' => 'goals',
            'language' => 'english',
        ]);

        $data['leads_followup'] = $this->emails_model->get([
            'type' => 'leads-follow-up',
            'language' => 'english',
        ]);

        $data['purchase'] = $this->emails_model->get([
            'type' => 'purchase',
            'language' => 'english',
        ]);

        $data['debit_notes'] = $this->emails_model->get([
            'type' => 'debit_note',
            'language' => 'english',
        ]);
    ...
    public function sent_smtp_test_email()
    {
        if ($this->input->post()) {
            ...
            $this->email->reply_to($staff->email, $userName);
    ...
    public function save_template_data()
    {
        ...
        $this->db->where('emailtemplateid', $this->input->post('id'));
    ...
    public function proposal_terms_reorder()
    {
        if ($this->input->post()) {
            $data = $this->input->post();
            $i = 1;
            foreach ($data['order'] as $id) {
            $success = $this->emails_model->update_data(["order" => $i], $id);
                $i++;
            }
            $result['success'] = true;
            $result['message'] =  "Proposal term order changed successfully";
        } else {
