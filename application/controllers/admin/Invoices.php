<?php
/*
ALTER TABLE `tblinvoices` ADD `commision_amount` VARCHAR(100) NULL DEFAULT NULL AFTER `discount_type`;
ALTER TABLE `tblinvoices` CHANGE `commision_amount` `commision_amount` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT '0';
*/
...
class Invoices extends AdminController
{
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if (isset($invoice_data['dynamic_fields'])) {
                $dynamic_amount_fields = $invoice_data['dynamic_fields'];
                unset($invoice_data['dynamic_fields']);
            }
            $tax_id = null;
            if (isset($invoice_data['tax_id'])) {
                $tax_id = $invoice_data['tax_id'];
                unset($invoice_data['tax_id']);
            }

            $sale_agent_id = $invoice_data['sale_agent'];

            $member = $this->staff_model->get($sale_agent_id);
            if ($member && is_numeric($sale_agent_id)) {
                $total_amount = $invoice_data['total'];
                $percent_rate = $member->hourly_rate;
                $commision_amount = calculateCommision($total_amount, $percent_rate);
                //$invoice_data['commision_amount'] = $commision_amount;
            }
            if ($id == '') {
                if (!has_permission('invoices', '', 'create')) {
                    access_denied('invoices');
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            $data['proposal_id'] = get_proposal_id_by_invoice($data['invoiceid']);
            $this->load->model('payments_model');
    ...
    public function send_to_email($id)
    {
        ...
        try {
            ...
            $success = $this->invoices_model->send_invoice_to_client(
                $id,
                '',
                $this->input->post('attach_tax_invoice'),
                $this->input->post('cc'),
                false,
                $statementData,
                $this->input->post('attach_packing_list'),
            );
        } catch (Exception $e) {
    ...
    public function delete($id)
    {
        ...
        if ($success) {
            ...
            log_message('error', 'Failed to delete Invoice ID: ' . $id);
    ...
    public function pdf($id)
    {
        ...
        if ($this->input->get('output_type')) {
            $action_type = $this->input->get('output_type');
        }

        $pdf->Output($invoice->pdf_type . "-" . mb_strtoupper(slug_it($invoice_number)) . '.pdf', $action_type);
    }
