...
class Staff extends AdminController
{
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if (isset($data['managerid'])) {
                    $data['managerid'] = implode(",", $data['managerid']);
                }
                ...
                handle_staff_profile_image_upload($id);
                $data['managerid'] = (isset($data['managerid'])) ? implode(",", $data['managerid']) : null;
                if (isset($data['manager_staff']) && is_array($data['manager_staff'])) {
                    $this->updateManagerStaffRelationships($id, $data['manager_staff']);
                    unset($data['manager_staff']);
                }
        ...
        $data['title']         = $title;
        $data['products']      = $this->staff_model->get_staffproducts($id);
    ...
    public function mail_service($id)
    {
        if ($this->input->post()) {
            $data = $this->input->post();
            $this->db->where('staffid', $id);
            $response = $this->db->update(db_prefix() . 'staff', $data);
            if ($response) {
                log_activity("Mail Service Credentials Updated ID [$id]");
                set_alert('success', _l('Mail service updated successfully'));
            }
            redirect(admin_url('staff/edit_profile'));
        }
    }
    ...
    /**
     * Update manager-staff relationships
     *
     * @param int $managerId The ID of the manager
     * @param array $newStaffIds Array of staff IDs that should be associated with this manager
     * @return array Returns counts of added and removed relationships
     */
    public function updateManagerStaffRelationships($managerId, $newStaffIds = [])
    {
        $result = [
            'added' => 0,
            'removed' => 0
        ...
        if (!is_array($newStaffIds)) {
            $newStaffIds = explode(',', $newStaffIds);
        }

        $newStaffIds = array_filter(array_map('trim', $newStaffIds), function ($value) {
            return !empty($value);
        });

        $this->db->select('staffid, managerid');
        $this->db->where("FIND_IN_SET('$managerId', managerid) >", 0);
        $query = $this->db->get(db_prefix() . 'staff');
        $currentStaffWithManager = [];

        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $currentStaffWithManager[] = $row->staffid;
            }
        }

        $staffToAdd = array_diff($newStaffIds, $currentStaffWithManager);
        $staffToRemove = array_diff($currentStaffWithManager, $newStaffIds);

        foreach ($staffToAdd as $staffId) {
            ...
            if ($query->num_rows() > 0) {
                $row = $query->row();
                $currentManagerIds = $row->managerid;
                $managerIdArray = empty($currentManagerIds) ? [] : explode(',', $currentManagerIds);

                if (!in_array($managerId, $managerIdArray)) {
                    $managerIdArray[] = $managerId;
                    $newManagerIds = implode(',', $managerIdArray);

                    $this->db->where('staffid', $staffId);
                    $update = $this->db->update(db_prefix() . 'staff', ['managerid' => $newManagerIds]);
        ...
        foreach ($staffToRemove as $staffId) {
            ...
            if ($query->num_rows() > 0) {
                $row = $query->row();
                $currentManagerIds = $row->managerid;
                $managerIdArray = explode(',', $currentManagerIds);

                $managerIdArray = array_diff($managerIdArray, [$managerId]);
                $newManagerIds = implode(',', $managerIdArray);
