...
class Estimates extends AdminController
{
    ...
    {
        if (!has_permission('estimates', '', 'view') && !has_permission('estimates', '', 'view_own') && !estimate_manager_permission_check($id) && get_option('allow_staff_view_estimates_assigned') == '0') {
    ...
    public function convert_to_invoice($id)
    {
        ...
        if ($invoiceid) {
            // Update all proposal advance payments to invoice if proposal linked with this estimate.
            $proposal = $this->proposals_model->get('',["estimate_id" =>  $id]);
            if(!empty($proposal)){
                $advance_payments = $this->payments_model->get_proposal_payments($proposal[0]['id']);
                if(!empty($advance_payments)){
                    foreach($advance_payments as $paymentData){
                        $this->payments_model->advance_payment_move_to_invoice($paymentData['paymentid'],$invoiceid);
                    }
                }
            }
