...
class Reports extends AdminController
{
    ...
    public function leads_weekly_report()
    {
        $data = $this->input->post();
        $dateRange = explode("/", $data['dateRange']);
        $data['success'] = true;
        $data['data'] = $this->reports_model->leads_week_report($dateRange[0], $dateRange[1]);
    ...
    public function source_report()
    {
        $data = $this->input->post();
        $data['success'] = true;
        $data['data'] = $this->reports_model->leads_sources_report($data);
        echo json_encode($data);
        exit;
    }

    public function leads_monthly_report()
    {
        $data = $this->input->post();
        $data['success'] = true;
        $data['data'] = $this->reports_model->leads_monthly_report($data);
        echo json_encode($data);
        exit;
    }
    ...
    public function lost_leads()
    {
        if ($this->input->is_ajax_request()) {
            $data = $this->input->post();
            $this->app->get_table_data('report_lost_leads', ["where" => $data]);
        }
    }

    public function leads_conversion()
    {
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('report_leads_conversion');
        }
    }
 ...
 public function payments_received()
{
    log_message('error', '--- payments_received() STARTED ---');

    if (!$this->input->is_ajax_request()) {
        log_message('error', 'Not an AJAX request - exiting.');
        return;
    }

    log_message('error', 'AJAX request confirmed.');

    // Load required models
    $this->load->model('currencies_model');
    $this->load->model('payment_modes_model');
    log_message('error', 'Models loaded: currencies_model, payment_modes_model.');

    // Fetch payment gateways
    $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
    log_message('error', 'Payment gateways fetched: ' . count($payment_gateways));

    // Define select columns (fully qualified where appropriate)
    $select = [
        db_prefix() . 'invoicepaymentrecords.id',
        db_prefix() . 'invoicepaymentrecords.date as payment_date',
        'invoiceid',
        get_sql_select_client_company(), // keep as-is (helper returns full client/company CASE)
        'paymentmode',
        'transactionid',
        'note',
        'amount',
    ];
    log_message('error', 'Select columns defined.');
    ...
    $custom_date_select = $this->get_where_report_period(db_prefix() . 'invoicepaymentrecords.date');
    log_message('error', 'Custom date filter: ' . $custom_date_select);
    if ($custom_date_select != '') {
        array_push($where, $custom_date_select);
    }
    ...
    if ($by_currency) {
        $currency = $this->currencies_model->get($by_currency);
        array_push($where, 'AND currency=' . $by_currency);
        log_message('error', 'Currency filter applied: ' . $by_currency);
    } else {
        $currency = $this->currencies_model->get_base_currency();
        log_message('error', 'Base currency used: ' . $currency->id);
    }

    $aColumns     = $select;
    $sIndexColumn = 'id';
    // Use backticked table name to avoid triggering the special-case ORDER BY in data_tables_init
    // This keeps the SQL valid while preventing the helper from adding an ambiguous "date" order clause.
    $sTable       = '`' . db_prefix() . 'invoicepaymentrecords`';
    ...
    log_message('error', 'JOINs defined.');

    // Add deleted_at filters
    array_push($where, 'AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL');
    array_push($where, 'AND ' . db_prefix() . 'invoices.deleted_at IS NULL');
    log_message('error', 'WHERE conditions: ' . json_encode($where));

    // Execute DataTables initialization
    try {
        ...
        log_message('error', 'data_tables_init executed successfully.');
    } catch (Exception $e) {
        log_message('error', 'data_tables_init FAILED: ' . $e->getMessage());
    ...
    log_message('error', 'Records fetched: ' . count($rResult));
    ...
    foreach ($rResult as $aRow) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            // Normalise key retrieval for columns aliased as "something as alias"
            $columnDef = $aColumns[$i];
            if (strpos($columnDef, ' as ') !== false) {
                // if columnDef like "tblinvoicepaymentrecords.date as payment_date", extract alias
                $alias = trim(strafter($columnDef, ' as '));
                $_data = isset($aRow[$alias]) ? $aRow[$alias] : (isset($aRow[$columnDef]) ? $aRow[$columnDef] : null);
            } else {
                // for fully qualified columns CI returns keys like "tblinvoicepaymentrecords.id" OR the aliasless column name
                if (isset($aRow[$columnDef])) {
                    $_data = $aRow[$columnDef];
                } else {
                    // fallback: try to extract field name from qualified column (after last dot)
                    $lastDot = strrpos($columnDef, '.');
                    if ($lastDot !== false) {
                        $fieldName = substr($columnDef, $lastDot + 1);
                        $_data = isset($aRow[$fieldName]) ? $aRow[$fieldName] : null;
                    } else {
                        $_data = isset($aRow[$columnDef]) ? $aRow[$columnDef] : null;
                    }
                }
            }

            // Format columns
            if ($columnDef == 'paymentmode') {
                // prefer name from joined payment_modes table
                $_data = isset($aRow['name']) ? $aRow['name'] : $_data;
                if (is_null(isset($aRow['paymentmodeid']) ? $aRow['paymentmodeid'] : null)) {
                    foreach ($payment_gateways as $gateway) {
                        if ((isset($aRow['paymentmode']) ? $aRow['paymentmode'] : null) == $gateway['id']) {
                            $_data = $gateway['name'];
                            break;
                        }
                    }
            ...
            } elseif ($columnDef == db_prefix() . 'invoicepaymentrecords.id') {
                $_data = '<a href="' . admin_url('payments/payment/' . $_data) . '" target="_blank">' . $_data . '</a>';

            } elseif ($columnDef == db_prefix() . 'invoicepaymentrecords.date' || (strpos($columnDef, ' as ') !== false && isset($alias) && $alias == 'payment_date')) {
                // Format payment_date
                $_data = _d($_data);

            } elseif ($columnDef == 'invoiceid') {
                $_data = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['invoiceid']) . '" target="_blank">' . format_invoice_number($aRow['invoiceid']) . '</a>';

            } elseif ($i == 3) {
                // client/company column (index based on select array)
                if (empty($aRow['deleted_customer_name'])) {
                    $_data = '<a href="' . admin_url('clients/client/' . $aRow['clientid']) . '" target="_blank">' . $aRow['company'] . '</a>';
                } else {
                    $_data = $aRow['deleted_customer_name'];
                }

            } elseif ($columnDef == 'amount') {
                $footer_data['total_amount'] += (float) $_data;
                $_data = app_format_money($_data, $currency->name);
            }
    ...
    log_message('error', 'Total amount calculated: ' . $footer_data['total_amount']);

    // Send final response
    echo json_encode($output);
    log_message('error', 'JSON output sent successfully.');
    log_message('error', '--- payments_received() COMPLETED ---');
    die();
}
    ...
    public function estimates_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            array_push($where, 'AND ' . db_prefix() . 'estimates.deleted_at IS NULL');
            $aColumns     = $select;
            $sIndexColumn = 'id';
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                ...
                '(SELECT ' . db_prefix() . 'creditnotes.total - (
                  (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.credit_id=' . db_prefix() . 'creditnotes.id AND ' . db_prefix() . 'credits.deleted_at IS NULL)
                  +
                  (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'creditnote_refunds WHERE ' . db_prefix() . 'creditnote_refunds.credit_note_id=' . db_prefix() . 'creditnotes.id AND ' . db_prefix() . 'creditnote_refunds.deleted_at IS NULL)
                  )
                ) as remaining_amount',
                'status',
            ];
            ...
            array_push($where, 'AND ' . db_prefix() . 'creditnotes.deleted_at IS NULL');
            if ($custom_date_select != '') {
                array_push($where, $custom_date_select);
            }
    ...
    public function invoices_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                ...
                'discount_total',
                'adjustment',
                '(SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'credits.deleted_at IS NULL) as credits_applied',
                '(SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid = ' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'credits.deleted_at IS NULL))',
                'status',
            ];
