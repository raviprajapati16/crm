...
class Settings extends AdminController
{
    ...
    public function index()
    {
        ...
        if ($this->input->post() || !empty($_FILES)) {
            ...
            $brochureUploaded = (handle_company_brochure_upload() ? true : false);
            ...
            if ($tab == 'email_campaigns_settings') {
                $post_data['settings']['email_campaign_operation_hours'] = (isset($tmpData['email_campaign_operation_hours'])) ? '1' : '0';
            }
    ...
    public function remove_signature_image_mpdf()
    {
        if (!has_permission('settings', '', 'delete')) {
            access_denied('settings');
        }
        ...
        redirect(admin_url('settings?group=mpdf'));
    }
    ...
    public function remove_company_logo($type = '')
    {
        ...
        if ($type == 'white') {
            $logoName = get_option('company_logo_white');
        }
        ...
        update_option(($type) ? 'company_logo_' . $type : 'company_logo', '');
        redirect($_SERVER['HTTP_REFERER']);
    }

    public function remove_company_brochure()
    {
        if (!has_permission('settings', '', 'delete')) {
            access_denied('settings');
        }

        $filename = get_option('company_brochure');
        ...
        update_option('company_brochure', '');
        redirect($_SERVER['HTTP_REFERER']);
    }
