...
class Clients extends ClientsController
{
    ...
    {
        return total_rows(db_prefix() . 'contacts', 'id !=' . get_contact_user_id() . ' AND email="' . $email . '" AND deleted_at IS NULL') > 0 ? false : true;
    }

    public function media($status = '')
    {
        $this->load->model('customer_media_model');
        $data['media'] = $this->customer_media_model->get_customer_media(get_client_user_id());
        $data['title']     = "Media";
        $data['bodyclass'] = 'media';
        $this->data($data);
        $this->view('media');
        $this->layout();
    }
    ...
    {
        if (!has_contact_permission('assigned_Leads')) {
            set_alert('warning', _l('access_denied'));
            redirect(site_url());
        }
        $this->load->model('leads_model');
        $data['leads'] = $this->leads_model->get_customer_leads(get_client_user_id());
        $data['title'] = "Assigned Leads";
        $this->data($data);
        $this->view('assigned_leads');
        $this->layout();
    }
