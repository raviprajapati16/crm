...
class App_mail_template
{
    ...
    public function send()
    {
        ...
        if (!empty($this->cc)) {
            if (is_array($this->cc)) {
                ...
                $cc_receipt = array_filter(explode(",", $this->cc));
            }
        }

        if (!in_array($this->template->slug, $slugArr)) {
            if (IS_LIVE && get_global_emails('cc')) {
                $ccGlobal = get_global_emails('cc');
                if (!empty($ccGlobal)) {
                    foreach ($ccGlobal as $ccGlobalRecipient) {
                        array_push($cc_receipt, $ccGlobalRecipient);
                    }
                }
            }
        }
        ...
        if (!empty($cc_receipt)) {
            $cc_receipt = array_unique($cc_receipt);
            if (!empty($cc_receipt)) {
                $this->ci->email->cc($cc_receipt);
            }
        }
        ...
        if (!empty($this->_reply_to())) {
            if (is_array($this->_reply_to())) {
                $replyTo = $this->_reply_to();
            } else {
                $replyTo = array_filter(explode(",", $this->_reply_to()));
            }
        }
            ...
            if (IS_LIVE && get_global_emails('bcc')) {
                ...
                if (!empty($bccGlobal)) {
                    foreach ($bccGlobal as $bccGlobalRecipient) {
                        array_push($replyTo, $bccGlobalRecipient);
                    }
                }
            }
        }
        ...
        if (is_staff_logged_in()) {
            ...
            if (!empty($staffData) && !empty($staffData->webmail_email)) {
                array_push($replyTo,  $staffData->webmail_email);
            }
        }
    ...
    {
        return isset($this->template->reply_to) ? $this->template->reply_to : $this->reply_to;
    }
    ...
    private function _bcc()
    {
        ...
        $bcc = get_global_emails('bcc');

        if (IS_LIVE && !empty($bcc)) {
    ...
    private function is_user_inactive()
    {
        ...
        if (in_array($this->slug, $checkSlugArr)) {
            if ($this->for === 'staff') {
            ...
            if ($inactive_user_table_check != '') {
                $this->ci->db->select('*')->where('email', $this->send_to)->where('active', 1);
                $count = $this->ci->db->count_all_results($inactive_user_table_check);
                if ($count > 0) {
                    return false;
                } else {
                    $this->clear();
    ...
    private function get_language($email, $template = null, $params = [])
    {
        ...
        if ($rel_type != 'proposal' && $rel_type != 'lead') {
            if ($for === 'customer' && total_rows(db_prefix() . 'contacts', ['email' => $email, 'deleted_at' => NULL]) > 0) {
