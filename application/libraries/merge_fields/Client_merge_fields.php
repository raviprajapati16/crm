...
class Client_merge_fields extends App_merge_fields
{
    ...
    public function format($client_id, $contact_id = '', $password = '')
    {
        ...
        $this->ci->db->order_by('is_primary', 'DESC');
        ...
        if (!empty($contacts)) {
            ...
            foreach ($contacts as $key => $contact) {
                ...
                $fields['{contact' . $index . '_firstname}'] = $contact->firstname;
                $fields['{contact' . $index . '_lastname}'] = $contact->lastname;
                $fields['{contact' . $index . '_email}'] = $contact->email;
                $fields['{contact' . $index . '_phonenumber}'] = $contact->phonenumber;
                $index++;
            }
        }
        ...
        $fields['{client_country}'] = get_country_name($client->country);
        ...
        if (isset($client->leadid) && !empty($client->leadid)) {
            $this->ci->load->model('leads_model');
            $lead_data = $this->ci->leads_model->get($client->leadid);
            $fields['{client_position}'] = $lead_data->title;
        }
