...
class App_sms
{
    ...
    private function set_default_triggers()
    {
        ...
        $triggers = [
            ...
            SMS_TRIGGER_CONTRACT_NEW_COMMENT_TO_CUSTOMER => [
                ...
                'label'        => 'New Comment on Agreement (to customer)',
    ...
    public function send_otp($phone, $otp)
    {
        ...
        if ($gateway !== false) {
            $className = 'sms_' . $gateway['id'];
            if (method_exists($this->ci->{$className}, 'send_otp')) {
                $retval = $this->ci->{$className}->send_otp($phone, $otp);
                return $retval;
            } else {
                return false;
            }
    ...
    public function verify_otp($phone, $otp)
    {
        ...
        if ($gateway !== false) {
            $className = 'sms_' . $gateway['id'];
            if (method_exists($this->ci->{$className}, 'verify_otp')) {
                $retval = $this->ci->{$className}->verify_otp($phone, $otp);
                return $retval;
            } else {
                return false;
            }
