...
<style>
    ...
    #cm-breadcrumb .breadcrumb-item+.breadcrumb-item::before {
        content: "›";
    }

    #cm-breadcrumb .breadcrumb-item a {
        color: #2196F3;
        cursor: pointer;
        text-decoration: none;
    }

    #cm-breadcrumb .breadcrumb-item a:hover {
        text-decoration: underline;
    }

    #cm-breadcrumb li.active {
        color: #555;
        font-weight: 600;
    }
    ...
    .cm-tooltip .cm-tt-name {
        font-weight: 700;
        font-size: 14px;
        margin-bottom: 4px;
    }

    .cm-tooltip .cm-tt-count {
        color: #1976D2;
    }
...
<script>
    ...
    var ClientMap = (function() {
        ...
        var COLOR_ACTIVE = '#f4951aff'; // flat color for regions with customers
        var COLOR_EMPTY = '#e8ecf0'; // flat color for regions with 0 customers
        var HOVER_COLOR = '<?php echo ($this->session->userdata("client_map_is_target_market") == "1") ? "#f9b146ff" : "#749e4c"; ?>';
        ...
        function init() {
            chart = echarts.init(document.getElementById('cm-chart'), null, {
                renderer: 'canvas'
            });
            ...
            document.getElementById('cm-export-csv').addEventListener('click', function() {
                _exportCsv();
            });
            document.getElementById('cm-export-city-csv').addEventListener('click', function() {
                _exportCsv(_listingExportLevel(cityContext));
            });
            document.getElementById('cm-export-pdf').addEventListener('click', function() {
                _exportPdf();
            });
            document.getElementById('cm-export-city-pdf').addEventListener('click', function() {
                _exportPdf(_listingExportLevel(cityContext));
            });
        ...
        function goBack() {
            if (currentLevel === 'state') {
                _loadLevel('country', currentISO2, null);
            } else if (currentLevel === 'country') {
                _loadLevel('world', null, null);
            }
        }
        ...
        function _renderMap(geojson, data, level, iso2, state) {
            ...
            chartData.forEach(function(d) {
                d.itemStyle = {
                    areaColor: d.value > 0 ? COLOR_ACTIVE : COLOR_EMPTY
                };
            });
