...
if ($payment->invoiceid == "0") {
	$proposalData = $this->proposals_model->get($payment->proposal_id);
}
?>
<div id="wrapper">
	<div class="content">
							...
							<div class="panel-body">
								<h4 class="no-margin">
									<?php if ($payment->invoiceid == "0") { ?>
										...
										<a href="<?php echo admin_url('proposals#' . $payment->proposal_id); ?>">
											<?php echo format_proposal_number($payment->proposal_id); ?>
										</a>
									<?php } else { ?>
					...
					<div class="panel-body">
						...
						<div class="row">
							...
							<div class="col-sm-6 text-right">
								<address>
									<?php if ($payment->invoiceid == "0") { ?>
										<?php echo format_proposal_info($payment->proposal_id, "html"); ?>
									<?php } else { ?>
									...
									<table class="table table-borderd table-hover">
										<thead>
											<tr>
												...
												<?php
												$proposal_payment_data = get_proposal_payment_data($payment->proposal_id);
												if ($proposal_payment_data['remaining_amount'] > 0) { ?>
													<th><span class="text-danger">Amount Due</span></th>
												<?php } ?>
											</tr>
										</thead>
										<tbody>
											<tr>
												<td><?php echo format_proposal_number($proposalData->id); ?></td>
												<td><?php echo _d($proposalData->date); ?></td>
												<td><?php echo app_format_money($proposalData->total, $proposalData->currency); ?></td>
												<td><?php echo app_format_money($payment->amount, $proposalData->currency); ?></td>
												<?php
												if ($proposal_payment_data['remaining_amount'] > 0) { ?>
													<td class="text-danger">
														<?php echo app_format_money($proposal_payment_data['remaining_amount'], $proposalData->currency); ?>
													</td>
												<?php } ?>
											</tr>
										</tbody>
									...
									<table class="table table-borderd table-hover">
										<thead>
											<tr>
												...
												<?php if (
													$payment->invoice->status != Invoices_model::STATUS_PAID
