    ...
    <div class="row">
     ...
     <?php if(has_permission('items','','create')){ ?>
       <div class="_buttons">
        ...
        <a href="#" class="btn btn-info pull-left mleft5" data-toggle="modal" id="sub_groups">Sub Groups</a>
...
<div class="modal fade" id="groups" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  foreach($items_groups as $group){ ?>
    <tr class="row-has-options" data-group-row-id="<?php echo $group['id']; ?>">
      <td data-order="<?php echo $group['id']; ?>"><?php echo $i; ?></td>
...
<!-- Sub Group Modal -->
<div class="modal fade" id="sub_groups_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      ...
      <div class="modal-body">
        <?php if(has_permission('items','','create')){ ?>
          <?php echo form_open('admin/invoice_items/add_subgroup',array('id'=>'sub_group_form')); ?>
          ...
          <div class="row">
              ...
              <div class="col-md-3">
                <?php echo render_input('name','Sub Group Name'); ?>
              </div>
              <div class="col-md-4">
                <?php echo render_textarea('description','invoice_item_add_edit_description'); ?>
              </div>
              <div class="col-md-1">
                <button class="btn btn-info sub-group-btn" type="submit" id="sub_group_btn"><?php echo _l('new_item_group'); ?></button>
              </div>
          </div>
          <?php echo form_close(); ?>
          <hr />
        ...
        <div class="row">
         <div class="container-fluid">
          <table class="table dt-table table-items-groups" data-order-col="1" data-order-type="asc">
           ...
           <tbody>
  <?php
  if ($items_sub_groups != "" && count($items_sub_groups) > 0) {
      $i = 1; // counter start
      foreach ($items_sub_groups as $subgroup) { ?>
        <tr class="row-has-options" data-group-row-id="<?php echo $subgroup['id']; ?>">
          <td data-order="<?php echo $subgroup['id']; ?>"><?php echo $i; ?></td>
          <td data-order="<?php echo $subgroup['group_name']; ?>"><?php echo $subgroup['group_name']; ?></td>
          <td data-order="<?php echo $subgroup['name']; ?>">
            <?php echo $subgroup['name']; ?>
            <div class="row-options">
              <?php if (has_permission('items', '', 'edit')) { ?>
                <a href="#" class="edit-item-subgroup" data-group-row-id="<?php echo $subgroup['id']; ?>">
                  <?php echo _l('edit'); ?>
                </a>
              <?php } ?>
              <?php if (has_permission('items', '', 'delete')) { ?>
                | <a href="<?php echo admin_url('invoice_items/delete_subgroup/'.$subgroup['id']); ?>" class="delete-item-group _delete text-danger">
                  <?php echo _l('delete'); ?>
                </a>
              <?php } ?>
            </div>
...
<script>
  $(function(){
    ...
    if(get_url_param('groups_modal')){
     ...
     if(get_url_param('subgroup_modal')){
       // Set time out user to see the message
       setTimeout(function(){
         $('#sub_groups_modal').modal('show');
       },1000);
     }
  ...
  $("#sub_groups").click(function(){
    $("#sub_group_form")[0].reset();
    $("#sub_group_form #group_id").selectpicker('refresh');
    $("#sub_group_form #sub_group_btn").text('New Group');
    $("#sub_groups_modal").modal('show');
  });

  $('body').on('click','.edit-item-subgroup',function(e){
    var group_id = $(this).data('group-row-id');
       $.get(admin_url+'invoice_items/edit_subgroup/'+group_id, function(response){

          if(response.status == true) {

              $("#sub_group_form #group_id").val(response.data.group_id);
              $("#sub_group_form #group_id").selectpicker('refresh');
              $("#sub_group_form #name").val(response.data.name);
              $("#sub_group_form #description").val(response.data.description);
              $("#sub_group_form #id").val(response.data.id);
              $("#sub_group_form #sub_group_btn").text('Update Group');
            }
