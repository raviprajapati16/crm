...
<div class="col-md-12 no-padding animated fadeIn">
    <div class="panel_s">
        <?php echo form_open(admin_url('debit_notes/' . (isset($refund) ? 'edit_refund' : 'create_refund') .'/'. (isset($refund) ? $refund->id.'/'.$refund->debit_note_id : $debit_note->id)), ['id'=>'debit_note_refund_form']); ?>
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <?php
                    echo render_input('amount', 'refund_amount', isset($refund) ? $refund->amount : $debit_note->remaining_debits, 'number', array('max'=>(!isset($refund) ? $debit_note->remaining_debits : $debit_note->remaining_debits + $refund->amount),'min'=>1)); ?>
                    <?php echo render_date_input('refunded_on', 'debit_date', isset($refund) ? _d($refund->refunded_on ) : _d(date('Y-m-d'))); ?>
...
<script>
   $(function(){
     init_selectpicker();
     init_datepicker();
     appValidateForm($('#debit_note_refund_form'),{amount:'required',refunded_on:'required'});
 });
</script>
