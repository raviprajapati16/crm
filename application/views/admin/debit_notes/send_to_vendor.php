...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-' . $debit_note->id; ?>" id="debit_note_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('admin/debit_notes/send_to_email/' . $debit_note->id); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Send Email To Vendor</h4>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            foreach ($vendors as $vendor) {
                                array_push($selected, $vendor['id']);
                            }
                            echo render_select('sent_to[]', $vendors, array('id', 'email', 'name'), 'invoice_estimate_sent_to_email', $selected, array('multiple' => true), array(), '', '', false);
                        ...
                        <hr />
                        <?php echo render_textarea('email_template_custom', '', $template->message, array(), array(), '', 'tinymce-' . $debit_note->id); ?>
                        <?php echo form_hidden('template_name', $template_name); ?>
                    </div>
                </div>

                <?php if (count($debit_note->attachments) > 0) { ?>
                    <hr />
                    <div class="row">
                        <div class="col-md-12">
