...
$where  = [];
if (!is_admin()) {
    array_push($where, 'AND (staff = ' . get_staff_user_id() . ' OR creator=' . get_staff_user_id() . ')');
}
...
array_push($where, 'AND ' . db_prefix() . 'reminders.deleted_at IS NULL');
array_push($where, 'AND ' . db_prefix() . 'invoices.deleted_at IS NULL');
array_push($where, 'AND ' . db_prefix() . 'proposals.deleted_at IS NULL');
array_push($where, 'AND ' . db_prefix() . 'estimates.deleted_at IS NULL');
array_push($where, 'AND ' . db_prefix() . 'leads.isDeleted = "false"');
