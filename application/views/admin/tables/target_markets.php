...
$aColumns = [
    ...
    '(SELECT short_name FROM ' . db_prefix() . 'countries
        WHERE country_id = ' . db_prefix() . 'clients.country LIMIT 1) as country',
    db_prefix() . 'clients.state as state',
];
...
if (!has_permission('customers', '', 'view')) {
    if (manager_employee_data_access_permission_check("customers")) {
        array_push($where, 'AND ' . db_prefix() . 'clients.userid IN (SELECT customer_id FROM ' . db_prefix() . 'customer_admins WHERE staff_id IN (' . get_manager_assigned_staff_ids('', true) . '))');
    } else {
...
foreach ($rResult as $aRow) {
    ...
    $countryLabel .= '<a href="' . $countryUrl . '">' . _l('view') . '</a>';
    ...
    $countryLabel .= '</div>';
    $row[] = $countryLabel;
    $row[] = ($aRow['state'] ? $aRow['state'] : '-');     // State from clients table
    // Company
