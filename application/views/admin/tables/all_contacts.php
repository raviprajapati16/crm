...
$aColumns = [
    'company',
    'email',
    'firstname', // will merge with lastname later in output
    'lastname',
...
// GDPR consent column if needed
if (is_gdpr() && $consentContacts == '1') {
    array_splice($aColumns, 2, 0, '1'); // Insert consent after email
}
...
array_push($where, 'AND ' . db_prefix() . 'contacts.deleted_at IS NULL');
...
//     $row[] = (!empty($aRow['company']) ? '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '">' . $aRow['company'] . '</a>' : '');
//     $row[] = '<a href="mailto:' . $aRow['email'] . '">' . $aRow['email'] . '</a>';
//     $row[] = '<img src="' . contact_profile_image_url($aRow['id']) . '" class="client-profile-image-small mright5"><a href="#" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' .
//         '<div class="row-options">' .
//         '<a href="#" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;">' . _l('edit') . '</a>' .
//         '</div>';
...
foreach ($rResult as $aRow) {
    ...
    // 1. Company + edit/delete
    $companyCol = (!empty($aRow['company'])
        ? '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '">' . $aRow['company'] . '</a>'
        : '');

    $companyCol .= '<div class="row-options">';

    // Edit link (same contact() JS you were using)
    $companyCol .= '<a href="#" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;">' . _l('edit') . '</a>';

    // GDPR export (if enabled)
    if (is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1' && is_admin()) {
        $companyCol .= ' | <a href="' . admin_url('clients/export/' . $aRow['id']) . '">'
            . _l('dt_button_export') . ' (' . _l('gdpr_short') . ')</a>';
    }

    // Delete link (with same permission rules)
    if (has_permission('customers', '', 'delete') || is_customer_admin($aRow['userid'])) {
        if ($aRow['is_primary'] == 0 || ($aRow['is_primary'] == 1 && $aRow['total_contacts'] == 1)) {
            $companyCol .= ' | <a href="' . admin_url('clients/delete_contact/' . $aRow['userid'] . '/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
        }
    }
    ...
    // 3. Name (only image + name link, no edit/delete here anymore)
    $rowName = '<img src="' . contact_profile_image_url($aRow['id']) . '" class="client-profile-image-small mright5">
                <a href="#" onclick="contact(' . $aRow['userid'] . ',' . $aRow['id'] . ');return false;">' .
    ...
    // 6. Last Login
    $row[] = (!empty($aRow['last_login'])
        ? '<span class="text-has-action is-date" data-toggle="tooltip" data-title="' . _dt($aRow['last_login']) . '">' . time_ago($aRow['last_login']) . '</span>'
        : '');

    // 7. Active toggle
    $outputActive = '<div class="onoffswitch">
                        <input type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') .
        ' data-switch-url="' . admin_url() . 'clients/change_contact_status"
                          name="onoffswitch"
                          class="onoffswitch-checkbox"
                          id="c_' . $aRow['id'] . '"
                          data-id="' . $aRow['id'] . '"' . ($aRow['active'] == 1 ? ' checked' : '') . '>
                        <label class="onoffswitch-label" for="c_' . $aRow['id'] . '"></label>
                     </div>';
    $outputActive .= '<span class="hide">' . ($aRow['active'] == 1
        ? _l('is_active_export')
        : _l('is_not_active_export')) . '</span>';
    ...
    // Row styling for unconfirmed registration
    $row['DT_RowClass'] = 'has-row-options';
    if ($aRow['registration_confirmed'] == 0) {
        $row['DT_RowClass'] .= ' alert-info requires-confirmation';
        $row['Data_Title'] = _l('customer_requires_registration_confirmation');
