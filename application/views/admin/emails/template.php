               ...
               <div class="panel-body">
                  ...
                  <div class="row">
                     <div class="col-md-12">
                        ...
                        <?php if ($template->type != "proposalterms") echo render_input('fromname', 'template_fromname', $template->fromname); ?>
                        <div
                        ...
                        <?php if ($template->type != "proposalterms") { ?>
                        ...
                        <?php foreach ($available_languages as $availableLanguage) {
                           ...
                           if (count($lang_template) > 0) {
                              $lang_used = false;
                              if (get_option('active_language') == $availableLanguage || total_rows(db_prefix() . 'staff', array('default_language' => $availableLanguage)) > 0 || total_rows(db_prefix() . 'clients', array('default_language' => $availableLanguage, 'deleted_at' => NULL)) > 0) {
                                 $lang_used = true;
                              }
                              $hide_template_class = '';
                              if ($lang_used == false) {
...
<script>
   $(function() {
      ...
      var typingTimer;
      var doneTypingInterval = 1000; // 1 second after stop typing
      var $status = $('<small id="autosave_status" style="margin-left:10px;color:#28a745;"></small>');
      $("h4.bold.font-medium").append($status);

      // listen to editor changes
      tinymce.activeEditor.on('input change keyup', function() {
         clearTimeout(typingTimer);
         $status.text("Saving...");
         typingTimer = setTimeout(save_template_data, doneTypingInterval);
      });

      function save_template_data() {
         var editor = tinyMCE.activeEditor;
         var data = {};
         data.id = "<?= $template->emailtemplateid ?>";
         data.content = editor.getContent();

         $.post(admin_url + 'emails/save_template_data', data).done(function(response) {
            try {
...
//       function save_template_data(manual) {
//          var editor = tinyMCE.activeEditor;
//          var data = {};
//          data.id = "<?= $template->emailtemplateid ?>";
//          data.content = editor.getContent();
