...
<script>
    window.addEventListener('load', function() {
        appValidateForm($('#contract-type-form'), {
            name: 'required'
        ...
        $.post(url, data).done(function(response) {
            ...
            if (response.success == true) {
                ...
                if ($('body').hasClass('contract') && typeof(response.id) != 'undefined') {
                    ...
                    //$('#from_main_type').find('option:last').after('<option value="'+response.id+'">'+response.name+'</option>').selectpicker('refresh');
                    //$('#to_main_type').find('option:last').after('<option value="'+response.id+'">'+response.name+'</option>').selectpicker('refresh');
