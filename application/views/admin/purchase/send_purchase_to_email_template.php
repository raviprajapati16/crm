...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-' . $purchase->id; ?>" id="purchase_send_to_vendor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('admin/purchase/send_to_email/' . $purchase->id); ?>
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">
                    <span class="edit-title">Send Email To Vendor</span>
                </h4>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="checkbox checkbox-primary">
                            ...
                            <label for="attach_pdf">Attach Purchase Order PDF</label>
                        </div>
                        <?php echo render_input('cc', 'CC'); ?>
                        <h5 class="bold"><?php echo _l('proposal_preview_template'); ?></h5>
                        <hr />
                        <?php echo render_textarea('email_template_custom', '', $template->message, array(), array(), '', 'tinymce-' . $purchase->id); ?>
                        <?php echo form_hidden('template_name', $template_name); ?>
                    </div>

                </div>
                <?php if (count($purchase->attachments) > 0) { ?>
                    <hr />
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    $(function() {
        $("body").on('click', '.close-send-template-modal', function() {
            $('#purchase_send_to_vendor').modal('hide');
        });
    });
</script>