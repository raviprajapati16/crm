...
$staffRole = $this->db->select('role')->where('staffid', $staffId)->get(db_prefix() . 'staff')->row();

foreach ($statuses as $status) {

    // Role 13 logic
    if (!empty($staffRole) && $staffRole->role == 13) {

        // Status 5 or 1: always show
        if (in_array($status, [5,1])) {
            ...
            // Other statuses: show only if addedfrom = current staff
            $total_pages = ceil($this->proposals_model->do_kanban_query($status,$this->input->get('search'),1,array(),true,$staffId)/get_option('proposals_pipeline_limit'));
            $proposals = $this->proposals_model->do_kanban_query($status,$this->input->get('search'),1,array(
                'sort_by'=>$this->input->get('sort_by'),
                'sort'=>$this->input->get('sort')
            ), $staffId);
