   ...
   <div class="row">
      <div class="col-md-8 task-single-col-left">
                        ...
                        <tr class="odd">
                           <td colspan="5">
                              ...
                              <div class="timesheet-duration hide">
                                 <div class="col-md-12">
                                    <i class="fa fa-question-circle pointer pull-left mtop2" data-toggle="popover"
         ...
         <?php if (has_permission('tasks', '', 'edit')) { ?><a href="#"
               ...
               class="pull-left mtop10 mleft5 font-medium-xs"><i class="fa fa-pencil-square-o"></i></a>
         <?php } ?>
         <div class="clearfix"></div>
         ...
         <div
            class="form-group no-mbot checklist-templates-wrapper simple-bootstrap-select task-single-checklist-templates<?php if (count($checklistTemplates) == 0) {
               echo ' hide';
            } ?>">
         ...
         <p class="hide text-muted no-margin" id="task-no-checklist-items">
            <?php echo _l('task_no_checklist_items_found'); ?>
         </p>
                     ...
                     foreach ($task->attachments as $attachment) { ?>
                        ...
                        <div data-num="<?php echo $i; ?>" data-commentid="<?php echo $attachment['comment_file_id']; ?>"
                           ...
                           <ul class="list-unstyled task-attachment-wrapper" data-placement="right" data-toggle="tooltip"
                              ...
                              <li
                                 class="mbot10 task-attachment<?php if (strtotime($attachment['dateadded']) >= strtotime('-16 hours')) {
                                    echo ' highlight-bg';
                                 } ?>">
                                 <div class="mbot10 pull-right task-attachment-user">
                                    ...
                                    if (empty($attachment['external'])) {
                                       ...
                                       (!empty($attachment['thumbnail_link']) || !empty($attachment['external']))
                                    ...
                                    <?php } else if (!empty($attachment['external']) && $attachment['external'] == 'gdrive') { ?>
                                          <a href="<?php echo $href_url; ?>" target="_blank" class="" data-toggle="tooltip"
                                             data-title="<?php echo _l('open_in_google'); ?>"><i class="fa fa-google"
                                                aria-hidden="true"></i></a>
                                    <?php }
         ...
         <div class="tasks-comments inline-block full-width simple-editor" <?php if (count($task->comments) == 0) {
            ...
            <button type="button" class="btn btn-info mtop10 pull-right hide" id="addTaskCommentBtn" autocomplete="off"
               data-loading-text="<?php echo _l('wait_text'); ?>"
      ...
      <div class="col-md-4 task-single-col-right">
         ...
         <h5 class="no-mtop task-info-created">
            <?php if (($task->addedfrom != 0 && $task->addedfrom != get_staff_user_id()) || $task->is_added_from_contact == 1) { ?>
               <small
                  class="text-dark"><?php echo _l('task_created_by', '<span class="text-dark">' . ($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)) . '</span>'); ?>
                  <i class="fa fa-clock-o" data-toggle="tooltip"
                     data-title="<?php echo _l('task_created_at', _dt($task->dateadded)); ?>"></i></small>
               <br />
            <?php } else { ?>
               <small
                  class="text-dark"><?php echo _l('task_created_at', '<span class="text-dark">' . _dt($task->dateadded) . '</span>'); ?></small>
            <?php } ?>
         </h5>
         ...
         <div class="task-info task-status task-info-status">
            <h5>
               <i
                  class="fa fa-<?php if ($task->status == Tasks_model::STATUS_COMPLETE) {
                     echo 'star';
         ...
         <div
            class="task-info task-info-due-date task-single-inline-wrap<?php if (!$task->duedate && !has_permission('edit', '', 'tasks')) {
               echo ' hide';
            } ?>"
            <?php if (!$task->duedate) {
            ...
            <h5>
               ...
               <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                  <input name="duedate" tabindex="-1" value="<?php echo _d($task->duedate); ?>" id="task-single-duedate"
                     class="task-info-inline-input-edit datepicker pointer task-single-inline-field" autocomplete="off"
         ...
         <hr class="task-info-separator" />
         <div class="clearfix"></div>
         <h4 class="task-info-heading font-normal font-medium-xs"><i class="fa fa-user-o" aria-hidden="true"></i>
            <?php echo _l('task_single_assignees'); ?></h4>
