...
$proposal_info = '<div style="color:#424242;">';
    ...
    $proposal_info .= "<br>Manager : ".get_staff_full_name($proposal->assigned);
$proposal_info .= '</div>';
...
$pdf->writeHTMLCell(($swap == '0' ? (($dimensions['wk'] / 2) - $dimensions['rm']) : ''), '', '', ($swap == '0' ? $y : ''), $proposal_info, 0, 0, false, true, ($swap == '1' ? 'R' : 'J'), true);
    ...
    $cellX = ($swap == '1') ? $dimensions['lm'] : ''; // Set the X-coordinate based on $swap
    $pdf->writeHTMLCell($cellWidth, $rowcount * 1, $cellX, ($swap == '1' ? $y : ''), $client_details, 0, 1, false, true, ($swap == '1' ? 'J' : 'R'), true);

    $pdf->writeHTMLCell(($dimensions['wk'] - ($dimensions['rm'] + $dimensions['lm'])), '', '', '', $number, 0, 1, false, true, 'L', true);
    $pdf->writeHTMLCell(($dimensions['wk'] - ($dimensions['rm'] + $dimensions['lm'])), '', '', '', $proposal->subject, 0, 1, false, true, 'L', true);

    $dates_info = _l('proposal_date') . ': ' . _d($proposal->date);

    if (!empty($proposal->open_till)) {
        $dates_info .= '<br />' . _l('proposal_open_till') . ': ' . _d($proposal->open_till);
    }

    $cellX_dates = ($swap == '1') ? '' : $dimensions['wk'] / 2; // Set the X-coordinate for right alignment

    $pdf->writeHTMLCell($cellWidth, $rowcount * 2, $cellX_dates, '', $dates_info, 0, 1, false, true, 'R', true);
...
$content_parts = explode('{page_break}', $proposal->content);
