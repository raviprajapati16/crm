...
if (get_option('exclude_invoice_from_client_area_with_draft_status') == 1) {
    $where_total['status !='] = 6;
}

// Always exclude status 5
$where_total['status !='] = 5;

$total_invoices = total_rows(db_prefix().'invoices', $where_total);
$total_open     = total_rows(db_prefix().'invoices', ['status'=>1, 'clientid'=>get_client_user_id(), 'deleted_at'=>NULL]);
$total_paid     = total_rows(db_prefix().'invoices', ['status'=>2, 'clientid'=>get_client_user_id(), 'deleted_at'=>NULL]);
$total_not_paid_completely = total_rows(db_prefix().'invoices', ['status'=>3, 'clientid'=>get_client_user_id(), 'deleted_at'=>NULL]);
$total_overdue  = total_rows(db_prefix().'invoices', ['status'=>4, 'clientid'=>get_client_user_id(), 'deleted_at'=>NULL]);
