...
class Staff_model extends App_Model
{
    ...
    public function get_staffproducts($id)
    {
        if (!empty($id)) {
            $query = $this->db->query("
                SELECT tblproducts.productid, tblproducts.name,
                tblstaff_products.productid AS staffproductid,
                tblstaff_products.staffid
                FROM tblproducts
                LEFT JOIN tblstaff_products
                ON tblproducts.productid = tblstaff_products.productid
                AND tblstaff_products.staffid = " . $this->db->escape($id) . "
        ...
        // $query = $this->db->query("SELECT tblproducts.productid, tblproducts.name, tblstaff_products.productid as staffproductid, tblstaff_products.staffid FROM tblproducts Left Join tblstaff_products ON tblproducts.productid = tblstaff_products.productid AND ( tblstaff_products.staffid = " . $id . ") Order by ProductId");
        // return $query->result_array();
    }
    /**
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('staffid', $id);
        $this->db->delete(db_prefix() . 'staff_products');
        if (isset($products)) {
            foreach ($products as $product) {
                $this->db->insert(db_prefix() . 'staff_products', [
                    'staffid' => $id,
                    'productid' => $product,
                ]);
                if ($this->db->affected_rows() > 0) {
        ...
        $current = $this->db->get_where(db_prefix() . 'staff', ['staffid' => $id])->row();

        $slug_firstname = isset($data['firstname']) ? $data['firstname'] : $current->firstname;
        $slug_lastname = isset($data['lastname']) ? $data['lastname'] : $current->lastname;

        $slug = trim($slug_firstname . '-' . $slug_lastname);
        if ($slug == '') {
            $slug = 'unknown-' . $id;
        }
        ...
        log_message('info', 'Media slug set for staff ID ' . $id . ': ' . $slug);
    ...
    public function get_all_webmails($is_active = true)
    {
        $this->db->select('*');
        $this->db->where('webmail_email !=', null);
        $this->db->where('webmail_email !=', '');
        if ($is_active) {
