...
class Tasks_model extends App_Model
{
    ...
    const STATUS_AWAITING_FEEDBACK = 3;
...
//         $this->db->where($where);
//         log_message('info', 'CRON Finished: recurring_tasks() at ' . $where);
...
public function do_kanban_query($status, $search = '', $page = 1, $count = false, $where = [])
{
    ...
    $this->db->select(
        'id,name,duedate,startdate,status,' 
        . get_sql_select_task_total_checklist_items() . ',' 
        . get_sql_select_task_total_finished_checklist_items() . ',
        (SELECT COUNT(id) FROM ' . db_prefix() . 'task_comments WHERE taskid=' . db_prefix() . 'tasks.id) as total_comments,
        (SELECT COUNT(id) FROM ' . db_prefix() . 'files WHERE rel_id=' . db_prefix() . 'tasks.id AND rel_type="task") as total_files,'
    ...
    // Apply any custom where conditions
    $this->db->where($where);

    // log_message('error', 'KANBAN WHERE: ' . print_r($where, true));

    // ✅ Only apply hide-project filter if no specific project is requested
    if (!(isset($where['rel_id']) && isset($where['rel_type']) && $where['rel_type'] === 'project')) {
