...
class Estimates_model extends App_Model
{
    ...
    public function get_attachments($estimate_id, $id = '')
    {
        if (is_numeric($id)) {
            ...
            $estimate = $this->db->get(db_prefix() . 'estimates')->row();
            $fileArr = get_customer_linked_files($estimate->clientid);
            return $fileArr;
        }
    }
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->update(db_prefix() . 'estimates', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
    ...
    public function send_estimate_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            if ($attachpdf) {
                ...
                set_mailing_constant();
                $pdf = estimate_mpdf($_pdf_estimate);
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $template = mail_template($template_name, $estimate, $contact, $cc, $staff->email);
