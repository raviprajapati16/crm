...
class Utilities_model extends App_Model
{
    ...
    $staffRole = $this->db->select('role')->where('staffid', $staffId)->get(db_prefix() . 'staff')->row()->role;

    // Check role 13 + full permission
    $hasAllPermissions = (
        has_permission('proposals', '', 'view') &&
        has_permission('proposals', '', 'create') &&
        has_permission('proposals', '', 'edit')
    );

    // Default permission check
    $noPermissionsQuery = get_proposals_sql_where_staff($staffId);

    $this->db->select('subject,id,hash,addedfrom,status,CASE WHEN open_till IS NULL THEN date ELSE open_till END as date', false);
    $this->db->from(db_prefix() . 'proposals');
    $this->db->where('status !=', 2, false);
    $this->db->where('status !=', 3, false);
    $this->db->where('deleted_at IS NULL');
    ...
    if ($client_data) {
        ...
        // ✅ Role 13 condition + all permissions
        if ($staffRole == 13 && $hasAllPermissions) {
            // Show all proposals with status 5 or 1
            // and only own proposals for other statuses
            $this->db->group_start();
            $this->db->where_in('status', [1, 5]);
            $this->db->or_group_start();
            $this->db->where_not_in('status', [1, 5]);
            $this->db->where('addedfrom', $staffId);
            $this->db->group_end();
            $this->db->group_end();

        } elseif (!$has_permission_proposals) {
    ...
    foreach ($proposals as $proposal) {
        if (($client_data && !$has_contact_permission_proposals) || 
            (!$client_data && !user_can_view_proposal($proposal['id']))) {
            continue;
        }
        ...
        if (!$client_data) {
            ...
            foreach ($available_reminders as $key) {
                if (get_option('show_' . $key . '_reminders_on_calendar') == 1 && !$ff || $ff && array_key_exists($key . '_reminders', $filters)) {
                    ...
                    ->where(db_prefix() . 'reminders.deleted_at IS NULL')
