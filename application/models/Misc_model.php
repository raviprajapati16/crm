...
class Misc_model extends App_Model
{
    ...
    {
        $lastId = $this->check_duplicate_reminder($data);
        if ($lastId) {
            return $lastId;
        }
    ...
    public function check_duplicate_reminder($data)
    {
        $seconds_ago = date('Y-m-d H:i:s', strtotime('-1 seconds'));
        $this->db->where('description', nl2br($data['description']));
        $this->db->where('rel_type', $data['rel_type']);
        $this->db->where('rel_id', $data['rel_id']);
        $this->db->where('created_at >=', $seconds_ago);
        $this->db->limit(1);
    ...
    public function delete_reminder($id)
    {
        ...
        if ($reminder->creator == get_staff_user_id() || is_admin()) {
            ...
            $this->db->update(db_prefix() . 'reminders', [
                ...
                'deleted_by' => get_staff_full_name(),
            ]);
            if ($this->db->affected_rows() > 0) {
    ...
    public function _search_leads($q, $limit = 0, $where = [], $is_vendor = false)
    {
        $result = [
            'result'         => [],
            'type'           => 'leads',
        ...
        if ($is_vendor) {
            ...
            $has_permission_view = has_permission('vendors', '', 'view');
        } else {
            $is_manager = manager_employee_data_access_permission_check("leads");
            $has_permission_view = has_permission('leads', '', 'view');
        }
        $permission_view = $has_permission_view;
        $manager_staff_ids = get_manager_assigned_staff_ids('', true);
        ...
        if (is_staff_member()) {
            ...
            if (!$permission_view && !$is_manager) {
                $this->db->where('(assigned = ' . get_staff_user_id() . ' OR addedfrom = ' . get_staff_user_id() . ' OR is_public=1)');
            } else if (!$permission_view && $is_manager) {
                $this->db->where('(assigned IN (' . $manager_staff_ids . ') ' . ' OR addedfrom IN (' . $manager_staff_ids . ') ' . ' OR is_public=1)');
            }
    ...
    public function _search_contact_book($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'contact_book',
            'search_heading' => "Contact Book",
        ];

        $permission_view = has_permission('contact_book', '', 'view');
    ...
    public function _search_credit_notes($q, $limit = 0)
    {
        ...
        if ($has_permission_view_credit_notes || $has_permission_view_credit_notes_own) {
            ...
            $this->db->where(db_prefix() . 'creditnotes.deleted_at IS NULL');
            $this->db->where(db_prefix() . 'clients.deleted_at IS NULL');
            $this->db->where(db_prefix() . 'contacts.deleted_at IS NULL');
            $this->db->where('(
    ...
    public function _search_estimates($q, $limit = 0)
    {
        ...
        if ($has_permission_view_estimates || $has_permission_view_estimates_own || get_option('allow_staff_view_estimates_assigned') == '1') {
            ...
            $this->db->where(db_prefix() . 'estimates.deleted_at IS NULL');
            $this->db->where(db_prefix() . 'clients.deleted_at IS NULL');
            $this->db->where(db_prefix() . 'contacts.deleted_at IS NULL');
    ...
    public function get_reminder_single($id)
    {
        $this->db->where('id', $id);
        return $this->db->get(db_prefix() . 'reminders')->row();
    }
    ...
    public function update_reminder($data, $id)
    {
        ...
        $this->db->update(db_prefix() . 'reminders', $data);
        if ($this->db->affected_rows() > 0) {
            $reminderData = $this->get_reminder_single($id);


            if ($reminderData->rel_type == 'lead') {
                $this->load->model('leads_model');
                $this->leads_model->log_lead_activity($reminderData->rel_id, ' Reminder Updated. ID ' . $id, false);
            }

            log_activity('Reminder Updated [ ID ' . $id . ' REL TYPE : ' . ucfirst($reminderData->rel_type) . ' REL ID : ' . $reminderData->rel_id . ']');
            return true;
        }
        return false;
    }

    public function get_lead_last_reminder($lead_id)
    {
        $this->db->select('id');
    ...
    public function lead_reminder_count($lead_id)
    {
        $this->db->where('rel_id', $lead_id);
        $this->db->where('rel_type', 'lead');
        $this->db->where('deleted_at IS NULL');
    ...
    public function get_duplicate_lead_reminders_count($rel_type, $rel_id)
    {
        $this->db->select('*');
        $this->db->from('tblreminders');
        $this->db->like('description', 'Duplicate Lead');
        $this->db->where('rel_id', $rel_id);
        $this->db->where('rel_type', $rel_type);
    ...
    public function get_email_tracking_mail($id)
    {
        $this->db->select('*');
        $this->db->where('id', $id);
        return $this->db->get(db_prefix() . 'tracked_mails')->row();
    }
}
