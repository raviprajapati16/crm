...
public function get_proposals_templates()
{
    ...
    // public function get_proposals_templates()
    // {
    //     return $this->db->query('SELECT * FROM ' . db_prefix() . 'emailtemplates  where type =' . "'proposalterms' and language='english' order by `order` ASC")->result_array();
    // }
    ...
    public function add($data)
    {
        ...
        if ($this->copy == false) {
            if ($data['type'] == '0') { // domestix
                $data['content'] = get_option('proposal_domestic_terms');
            } else { //international
                $data['content'] = get_option('proposal_international_terms');
            }
        }
    ...
    public function update($data, $id)
    {
        ...
        if ($current_proposal->type != $data['type']) {
            if ($data['type'] == '0') { // domestix
                $data['content'] = get_option('proposal_domestic_terms');
            } else { //international
                $data['content'] = get_option('proposal_international_terms');
    ...
    {
        $this->db->where($where);
        $this->db->where(db_prefix() . 'proposals.deleted_at IS NULL');
    ...
    //     } else {
    //         $fileArr = [];
    //         //get proposal linked files
    //         $fileArr[] = get_attachments_by_type('proposal', $proposal_id);

    //         //get connected module files
    //         $this->db->select('*');
    //         $this->db->where('id', $proposal_id);
    //         $proposal = $this->db->get(db_prefix() . 'proposals')->row();


    //         //get connected lead linked files
    //         if ($proposal->rel_type == "lead") {
    //             $fileArr[] = get_attachments_by_type('lead', $proposal->rel_id);
    ...
    //             }
    //         } else { //customer
    //             // log_message('error', 'Customerrrrrrrrr: ' . json_encode($proposal));
    //             $fileArr[] = get_customer_linked_files($proposal->rel_id);

    //         }
    //         $fileArr = array_merge(...$fileArr);
    //         $fileArr = attachement_unique_filter($fileArr);
    //         return $fileArr;
    //     }
    // }
    ...
    public function get_attachments($proposal_id, $id = '')
    {
        ...
        // 1. Get files directly linked to proposal
        $proposal_files = get_attachments_by_type('proposal', $proposal_id);
        if (!empty($proposal_files)) {
            $fileArr[] = $proposal_files;
        }
        ...
        if ($proposal) {
            if ($proposal->rel_type == "lead") {
                // Attachments from the lead
                $lead_files = get_attachments_by_type('lead', $proposal->rel_id);
                if (!empty($lead_files)) {
                    $fileArr[] = $lead_files;
                }

                // Check if that lead is converted to a customer
                $this->db->where('leadid', $proposal->rel_id);
                $client = $this->db->get(db_prefix() . 'clients')->row();
                if (!empty($client)) {
                    $customer_files = get_customer_linked_files($client->userid);
                    if (!empty($customer_files)) {
                        $fileArr[] = $customer_files;
                    }
                }
            } elseif ($proposal->rel_type == "customer") {
                // Attachments linked to the customer

                $customer_files = get_customer_linked_files($proposal->rel_id, $proposal->rel_type);
        ...
        // Merge and filter unique attachments
        $fileArr = array_merge(...$fileArr);
        $fileArr = attachement_unique_filter($fileArr);
    ...
    public function delete($id)
    {
        ...
        $data['proposal_number_prefix'] = null;
    ...
    public function send_proposal_to_email($id, $attachpdf = true, $cc = '')
    {
        ...
        $sent = send_mail_template('proposal_send_to_customer', $proposal, $attachpdf, $cc, $staff->email);
    ...
    public function get_proposal_by_customer_id($id)
    {
        $viewPermission = has_permission('proposals', '', 'view');
        $clientData = $this->clients_model->get($id);
        $proposalUserCondition = get_proposals_sql_where_staff(get_staff_user_id());
        ...
        if (!empty($clientData)) {
            ...
            $this->db->where('deleted_at IS NULL');
            if (!$viewPermission) {
                $this->db->where($proposalUserCondition);
            }
            $this->db->group_start();
            $this->db->group_start();
            $this->db->where('rel_type', 'customer');
            $this->db->where('rel_id', $clientData->userid);
            $this->db->group_end();
            if (!empty($clientData->leadid)) {
                $this->db->or_group_start();
                $this->db->where('rel_type', 'lead');
                $this->db->where('rel_id', $clientData->leadid);
                $this->db->group_end();
            }
            $this->db->group_end();
    ...
    public function get_proposal_payments($proposal_id)
    {
        ...
        $this->db->select('invoice_id,estimate_id');
        $proposalData = $this->db->from(db_prefix() . 'proposals')->get()->row();
        if (!empty($proposalData->invoice_id)) {
            $invoice_id = $proposalData->invoice_id;
        } else if (!empty($proposalData->estimate_id)) {
            $this->db->where('id', $proposalData->estimate_id);
        ...
        if (!empty($invoice_id)) {
            ...
            return $this->payments_model->get_invoice_payments($invoice_id);
        } else {
            $this->load->model('payments_model');
            return $this->payments_model->get_proposal_payments($proposal_id);
        }
    }

    public function get_multiple_proposals($id = [])
    {
        $this->db->where_in(db_prefix() . 'proposals.id', $id);
        $this->db->select('*,' . db_prefix() . 'currencies.id as currencyid, ' . db_prefix() . 'proposals.id as id, ' . db_prefix() . 'currencies.name as currency_name');
        $this->db->from(db_prefix() . 'proposals');
        $this->db->join(db_prefix() . 'currencies', db_prefix() . 'currencies.id = ' . db_prefix() . 'proposals.currency', 'left');
        return $this->db->get()->result_array();
