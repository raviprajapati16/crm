...
class Invoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' . db_prefix() . 'invoices JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid=' . db_prefix() . 'invoices.sale_agent WHERE ' . db_prefix() . 'invoices.deleted_at IS NULL AND sale_agent != 0')->result_array();
    }
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->where(db_prefix() . 'invoices.deleted_at IS NULL');
    ...
    public function unmark_as_cancelled($id)
    {
        ...
        $installmentExists = $this->db->get(db_prefix() . 'invoicepaymentrecords')->row();
        $status = self::STATUS_UNPAID;
        if (!empty($installmentExists)) {
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE ' . db_prefix() . 'invoicepaymentrecords.invoiceid = ' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL AND ' . db_prefix() . 'invoices.deleted_at IS NULL) - (SELECT COALESCE(SUM(amount),0) FROM ' . db_prefix() . 'credits WHERE ' . db_prefix() . 'credits.invoice_id=' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'credits.deleted_at IS NULL AND ' . db_prefix() . 'invoices.deleted_at IS NULL)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' . db_prefix() . 'invoicepaymentrecords WHERE invoiceid=' . db_prefix() . 'invoices.id AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL AND ' . db_prefix() . 'invoices.deleted_at IS NULL) as total_paid';
            }
    ...
    public function update($data, $id)
    {
        ...
        if (count($items) > 0) {
            foreach ($items as $key => $item) {
                ...
                if (update_sales_item_post($item['itemid'], $item, 'hsn_code')) {
                    $affectedRows++;
                }

                if (update_sales_item_post($item['itemid'], $item, 'kind_of_packages')) {
                    $affectedRows++;
                }

                if (update_sales_item_post($item['itemid'], $item, 'net_weight')) {
                    $affectedRows++;
                }

                if (update_sales_item_post($item['itemid'], $item, 'gross_weight')) {
                    $affectedRows++;
                }
    ...
    public function get_attachments($invoiceid, $id = '')
    {
        if (is_numeric($id)) {
            ...
            $fileArr = get_customer_linked_files($invoice->clientid);
            return $fileArr;
        }
    }
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'invoices', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name(), "prefix" => null, "number" => null]);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->select('id');
            $this->db->where('id IN (SELECT credit_id FROM ' . db_prefix() . 'credits WHERE invoice_id=' . $id . ' AND deleted_at IS NULL)');
            $this->db->where('deleted_at IS NULL');
    ...
    public function send_invoice_to_client($id, $template_name = '', $attachTaxInvoice = true, $cc = '', $manually = false, $attachStatement = [],  $attachPackingList = true)
    {
        set_time_limit(0);
        $invoice = $this->get($id);
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            if (isset($attachStatement['attach']) && $attachStatement['attach'] == true) {
                ...
                $statementPdf = statement_mpdf($statement);
            ...
            if ($attachTaxInvoice) {
                $_pdf_invoice->pdf_type = 'tax-invoice';
                set_mailing_constant();
                $pdf    = invoice_mpdf($_pdf_invoice);
                $tax_invoice_pdf = $pdf->Output("tax-invoice-" . $invoice_number . '.pdf', 'S');
            }

            if ($attachPackingList) {
                $_pdf_invoice->pdf_type = 'packing-list';
                set_mailing_constant();
                $pdf    = invoice_mpdf($_pdf_invoice);
                $packing_list_pdf = $pdf->Output("packing-list-" . $invoice_number . '.pdf', 'S');
            }
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $template = mail_template($template_name, $invoice, $contact, $cc, $staff->email);
                    if ($attachTaxInvoice) {
                        $template->add_attachment([
                            'attachment' => $tax_invoice_pdf,
                            'filename'   => "tax-invoice-" . str_replace('/', '-', $invoice_number . '.pdf'),
                            'type'       => 'application/pdf',
                        ]);
                    }
                    if ($attachPackingList) {
                        $template->add_attachment([
                            'attachment' => $packing_list_pdf,
                            'filename'   => "packing-list-" . str_replace('/', '-', $invoice_number . '.pdf'),
                            'type'       => 'application/pdf',
                        ]);
                    }
