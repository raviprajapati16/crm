...
class Statement_model extends App_Model
{
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        $sql .= ' AND ' . db_prefix() . 'invoices.deleted_at IS NULL';
        if ($from == $to) {
            $sqlDate = 'date="' . $from . '"';
        } else {
        ...
        FROM ' . db_prefix() . 'creditnotes WHERE  ' . db_prefix() . 'creditnotes.deleted_at IS NULL AND clientid =' . $customer_id . ' AND status != 3';
        ...
        $sql_credits_applied .= '
        WHERE ' . db_prefix() . 'creditnotes.deleted_at IS NULL AND ' . db_prefix() . 'credits.deleted_at IS NULL AND clientid =' . $customer_id;
        ...
        WHERE ' . db_prefix() . 'invoices.deleted_at IS NULL AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL
        AND ' . $sqlDatePayments . ' AND ' . db_prefix() . 'invoices.clientid = ' . $customer_id . '
        ...
        WHERE ' . $sqlCreditNoteRefunds . ' AND ' . db_prefix() . 'creditnote_refunds.deleted_at IS NULL AND credit_note_id IN (SELECT id FROM ' . db_prefix() . 'creditnotes WHERE ' . db_prefix() . 'creditnotes.deleted_at IS NULL AND clientid=' . $customer_id . ')
        ';
        ...
        WHERE ' . db_prefix() . 'creditnotes.deleted_at IS NULL AND clientid = ' . $customer_id . '
        ...
        FROM ' . db_prefix() . 'invoicepaymentrecords
        JOIN ' . db_prefix() . 'invoices ON ' . db_prefix() . 'invoices.id = ' . db_prefix() . 'invoicepaymentrecords.invoiceid
        WHERE ' . $sqlDatePayments . ' AND ' . db_prefix() . 'invoices.deleted_at IS NULL AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL AND ' . db_prefix() . 'invoices.clientid = ' . $customer_id)
            ->row()->amount_paid;
        ...
        $result['beginning_balance'] = $this->db->query('
            ...
            AND ' . db_prefix() . 'invoices.deleted_at IS NULL AND ' . db_prefix() . 'invoicepaymentrecords.deleted_at IS NULL
            ) + (
                SELECT COALESCE(SUM(' . db_prefix() . 'creditnotes.total),0)
                FROM ' . db_prefix() . 'creditnotes
                WHERE ' . db_prefix() . 'creditnotes.date < "' . $from . '"
                AND ' . db_prefix() . 'creditnotes.deleted_at IS NULL AND ' . db_prefix() . 'creditnotes.clientid=' . $customer_id . '
            )
        )
            )
