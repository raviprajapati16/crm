...
class Clients_model extends App_Model
{
    ...
    {
        $fieldArr = array_merge(prefixed_table_fields_array(db_prefix() . 'clients'), prefixed_table_fields_array(db_prefix() . 'contacts'));
        $this->db->select(implode(',', $fieldArr) . ',' . get_sql_select_client_company());
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        if ($affectedRows > 0) {
            // Update GST in lead table.
            $clientData = $this->get($id);
            if (!empty($clientData->leadid)) {
                $this->db->where('id', $clientData->leadid);
    ...
    {
        return $this->db->query('SELECT DISTINCT(staff_id) FROM ' . db_prefix() . 'customer_admins where deleted_at IS NULL')->result_array();
    }
    ...
    public function delete($id)
    {
        ...
        $this->db->update(db_prefix() . 'clients', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
    ...
    public function delete_contact($id)
    {
        ...
        $this->db->update(db_prefix() . 'contacts', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
        if ($this->db->affected_rows() > 0) {
