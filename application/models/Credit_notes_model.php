...
class Credit_notes_model extends App_Model
{
    ...
    public function send_credit_note_to_client($id, $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            if ($attachpdf) {
                ...
                $pdf    = credit_note_mpdf($credit_note);
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $template = mail_template('credit_note_send_to_customer', $credit_note, $contact, $cc, $staff->email);
    ...
    public function get_attachments($credit_note_id)
    {
        //get credit note
        $this->db->where('id', $credit_note_id);
        $creditnote = $this->db->get(db_prefix() . 'creditnotes')->row();
        //get customer linked files including credit note
        $fileArr = get_customer_linked_files($creditnote->clientid);
        return $fileArr;
    }
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'creditnotes', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
    ...
    public function delete_applied_credit($id, $credit_id, $invoice_id)
    {
        $this->db->where('id', $id);
        //$this->db->delete(db_prefix() . 'credits');
        $this->db->update(db_prefix() . 'credits', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
        if ($this->db->affected_rows() > 0) {
    ...
    public function get_refunds($credit_note_id)
    {
        ...
        $this->db->where(db_prefix() . 'creditnote_refunds.deleted_at IS NULL');
    ...
    public function delete_refund($refund_id, $credit_note_id)
    {
        ...
        //$this->db->delete(db_prefix() . 'creditnote_refunds');
        $this->db->update(db_prefix() . 'creditnote_refunds', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
        if ($this->db->affected_rows() > 0) {
    ...
    private function total_credits_used_by_credit_note($id)
    {
        return sum_from_table(db_prefix() . 'credits', [
            'field' => 'amount',
            'where' => ['credit_id' => $id, 'deleted_at' => NULL],
        ]);
    }
