...
class Invoice_items_model extends App_Model
{
    ...
    public function get($id = '')
    {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid,rate,
            ' . db_prefix() . 'items.capacity as capacity,
            t1.taxrate as taxrate,t1.id as taxid,t1.name as taxname,
            t2.taxrate as taxrate_2,t2.id as taxid_2,t2.name as taxname_2,
            description,long_description,group_id,subgroup_id,' . db_prefix() . 'items_groups.name as group_name,unit,hsn_code,net_weight,gross_weight');
        ...
        $this->db->where(db_prefix() . 'items_groups.deleted_at IS NULL');
    ...
    public function add($data)
    {
        ...
        if (isset($data['item_sub_group']) && $data['item_sub_group'] != "") {
            $data['subgroup_id'] = $data['item_sub_group'];
    ...
    public function delete_group($id)
    {
        ...
        if ($group) {
            ...
            $this->db->update(db_prefix() . 'items_groups', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
    ...
    public function get_item_by_group($id, $subgroup_id = "")
    {
        $this->db->where('group_id', $id);
        if ($subgroup_id != "") {
    ...
    {
        $this->db->select(db_prefix() . 'sub_groups.*,' . db_prefix() . 'items_groups.name as group_name');
        $this->db->join(db_prefix() . 'items_groups', '' . db_prefix() . 'items_groups.id = ' . db_prefix() . 'sub_groups.group_id', 'left');
        $this->db->order_by(db_prefix() . 'sub_groups.id', 'desc');
        $this->db->where(db_prefix() . 'items_groups.deleted_at IS NULL');
        $this->db->where(db_prefix() . 'sub_groups.deleted_at IS NULL');
    ...
    public function add_subgroup($data)
    {
        $data['created_at'] = date('Y-m-d H:i:s');
        $data['created_by'] = get_staff_user_id();
        $this->db->insert(db_prefix() . 'sub_groups', $data);
    ...
    public function get_subgroup_by_id($id)
    {
        ...
        return $this->db->get(db_prefix() . 'sub_groups')->row();
    }
    ...
    public function update_subgroup($data)
    {
        ...
        $this->db->update(db_prefix() . 'sub_groups', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Sub Group Updated [Name: ' . $data['name'] . ']');
    ...
    {
        $this->db->where('id', $id);
        $group = $this->db->get(db_prefix() . 'sub_groups')->row();

        if ($group != "") {

            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'sub_groups', ["deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()]);
            //$this->db->delete(db_prefix() . 'sub_groups');

            log_activity('Sub Group Deleted [Name: ' . $group->name . ']');
