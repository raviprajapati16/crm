...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $leadifetch_status = $this->indiamart_fetchleads();
            log_activity($leadifetch_status);

            $leadiassign_status = $this->indiamart_assignleads();
            log_activity($leadiassign_status);

            $leadfetch_status = $this->tradeindia_fetchleads();
            log_activity($leadfetch_status);

            $leadassign_status = $this->tradeindia_assignleads();
            log_activity($leadassign_status);
            ...
            $this->delete_temp_lead_inquiry_forms();
    ...
    private function indiamart_fetchleads()
    {
        $CI = &get_instance();
        //$model = admin_url(INDIAMART_MODULE_NAME.'/indiamart_model');
        $CI->indiamart_setting = $this->Indiamartnew_model->get_settings();
        $response = $this->Indiamartnew_model->indiamart_curl_request();
        //$CI->$CI->load->model('Model File');
        ...
        if (!empty($response['result'])) {
            ...
            //$table_rows = '';
            $leadresponse_arr = $result_arr["RESPONSE"];
            if ($response['status']) {
                for ($i = 0; $i < count($leadresponse_arr); $i++) {
                    $query_id = $leadresponse_arr[$i]['UNIQUE_QUERY_ID'];
                    $lead_data = $leadresponse_arr[$i];
        ...
        return "Indiamart leads imported at " . date('d M Y H:i:s');
    }

    private function indiamart_assignleads()
    {
        $assignment_status = "No new Indiamart Leads.";
        $data['leads'] = $this->Indiamartnew_model->getLeads();
        $source = "3";
        $status = "2";
        if (sizeof($data['leads']) > 0) {
            foreach ($data['leads'] as $leadRow) {
                try {
                    if ($leadRow) {
            ...
            $response['message'] = count($imported_leads) . " IndiaMart Lead(s) Imported Successfully";
            $response['message_type'] = "success";
            $assignment_status = $response['message'];
        }
        return $assignment_status;
    }

    public function tradeindia_fetchleads($param = [])
    {
        $CI = &get_instance();
        $CI->tradeindiasettings = $this->TradeIndia_model->get_settings();
        $response = $this->TradeIndia_model->tradeindia_curl_request($param);
        ...
        return "TradeIndia leads imported at " . date('d M Y H:i:s');
    }

    public function tradeindia_assignleads()
    {
        $assignment_status = "No new TradeIndia Leads.";
        $source = "15";
        $status = "2";
        $data['leads'] = $this->TradeIndia_model->getLeads();
        if (sizeof($data['leads']) > 0) {
            foreach ($data['leads'] as $leadRow) {
                try {
                    if ($leadRow) {
                    ...
                    //$log = "Error while processing lead: " . $leadRow->id . ": " . $th;
                    log_activity($th);
                }
            } /* Leads Loop */
            // $response['status'] = TRUE;
            // $response['imported_leads'] = $imported_leads;
            $response['message'] = count($imported_leads) . " TradeIndia Lead(s) Imported Successfully";
            // $response['message_type'] = "success";
            $assignment_status = $response['message'];
        } // If Leads are avalible
        return $assignment_status;
    } // Trade India Assign Leads

    // public function googlesheet_assignleads()
    // {
    ...
    //             $columnMapping = json_decode($sheet['column_mapping'], true);
    //             $records = $this->Google_sheets_model->get_sheet_records($sheet['id'], false);
    //             $imported_leads = [];
    //             foreach ($records as $key1 => $record) {
    //                 $recordData = json_decode($record['record_data'], true);
    //                 $leadData = array();
    //                 if (!empty($recordData)) {
    //                     $importantFields = ['name', 'email', 'phonenumber', 'company', 'address', 'country', 'state', 'city', 'zipcode', 'product', 'website', 'description'];
    //                     foreach ($importantFields as $field) {
    //                         if (isset($columnMapping[$field]) && !empty($columnMapping[$field])) {
    //                             $text = $recordData[$columnMapping[$field]];
    //                             $leadData[$field] = str_replace("p:", "", $text);
    //                         } else {
    //                             $leadData[$field] = "";
    //                         }
    //                     }
    ...
    //                 if (!empty($leadData)) {
    //                     $mobileNumbers = isset($leadData['phonenumber']) ? $leadData['phonenumber'] : '';
    //                     $mobileNumbersArray = !empty($mobileNumbers) ? explode(', ', $mobileNumbers) : [];
    //                     $emaildata = isset($leadData['email']) ? $leadData['email'] : '';
    //                     $emaildataArray = !empty($emaildata) ? explode(', ', $emaildata) : [];
    //                     $countryId = 0;
    //                     if (!empty($leadData['country'])) {
    //                         $this->db->where('iso2', $leadData['country']);
    //                         $this->db->or_where('short_name', $leadData['country']);
    //                         $this->db->or_where('long_name', $leadData['country']);
    //                         $country = $this->db->get(db_prefix() . 'countries')->row();
    //                         if ($country) {
    ...
    //                     }
    //                     $insert_data = [
    //                         'name' => isset($leadData['name']) ? $leadData['name'] : '',
    //                         'source' => $source,
    //                         'status' => $status,
    //                         'email' => implode(',', $emaildataArray),
    //                         'phonenumber' => implode(',', $mobileNumbersArray),
    //                         'title' => '',
    //                         'tags' => $leadData['product'],
    //                         'company' => isset($leadData['company']) ? $leadData['company'] : '',
    //                         'website' =>  isset($leadData['website']) ? $leadData['website'] : '',
    //                         'address' => isset($leadData['address']) ? $leadData['address'] : '',
    //                         'city' => isset($leadData['city']) ? $leadData['city'] : '',
    //                         'zip' => isset($leadData['zipcode']) ? $leadData['zipcode'] : '',
    //                         'state' => isset($leadData['state']) ? $leadData['state'] : '',
    //                         'description' => isset($leadData['description']) ? $leadData['description'] : '',
    //                         'country' => $countryId,
    //                         'assigned' => $assignee,
    //                     ];
    //                     $output = $this->leads_model->add($insert_data);
    //                     if ($output) {
    //                         $this->Google_sheets_model->update_sheet_records($record['id'], ['is_imported' => 1]);
    //                         $imported_leads[] = $record['id'];
    //                     }
    //                 }
    //             }
    //             if (!empty($imported_leads)) {
    //                 count($imported_leads) . " Imported Successfully From Google sheet ID [" . $sheet['id'] . "]";
    //             }
    //         }
    //     }
    // }
    public function googlesheet_assignleads()
    {
        log_message('debug', 'Starting Google Sheet Leads Import Process.');

        $sheets = $this->Google_sheets_model->get_sheets();
        if (!empty($sheets)) {
            foreach ($sheets as $key => $sheet) {
                log_message('debug', "Processing sheet ID: {$sheet['id']}");
                ...
                log_message('debug', 'Fetching sheet records...');
                $records = $this->Google_sheets_model->get_sheet_records($sheet['id'], false);
                ...
                foreach ($records as $key1 => $record) {
                    ...
                    log_message('debug', "Mapped lead data: " . json_encode($leadData));
                    ...
                    if (!empty($leadData)) {
                        ...
                        if (!empty($leadData['country'])) {
                            ...
                            if ($country) {
                                $countryId = $country->country_id;
                                log_message('debug', "Found country: {$country->long_name} (ID: {$countryId})");
                                ...
                                log_message('debug', "Country not found for: " . $leadData['country']);
                            }
                        }
                        ...
                        log_message('debug', 'Inserting lead: ' . json_encode($insert_data));
                        $output = $this->leads_model->add($insert_data);

                        if ($output) {
                            log_message('debug', "Lead inserted successfully with ID: {$output}");
                            $this->Google_sheets_model->update_sheet_records($record['id'], ['is_imported' => 1]);
                            $imported_leads[] = $record['id'];
                        } else {
                            log_message('debug', 'Failed to insert lead.');
                        }
                    }
                }

                if (!empty($imported_leads)) {
                    log_message('debug', count($imported_leads) . " leads imported from sheet ID: {$sheet['id']}");
                } else {
                    log_message('debug', "No leads imported from sheet ID: {$sheet['id']}");
                }
            }
        } else {
            log_message('debug', 'No Google sheets configured.');
        }

        log_message('debug', 'Google Sheet Leads Import Process completed.');
    }
    ...
    public function contracts_expiration_check()
    {
        ...
        $this->db->select('id,client,dateend,subject,addedfrom,not_visible_to_client,rel_type,dateadded');
                    ...
                    if ($contract['not_visible_to_client'] == 0) {
                        ...
                        foreach ($contacts as $contact) {
                            if ($contract['rel_type'] == "customer") {
                                ...
                                $template = mail_template('contract_expiration_reminder_to_vendor', $contract, $contact);
                            } else if ($contract['rel_type'] == "contact_book") {
                                $template = mail_template('Contract_expiration_reminder_to_contact_book', $contract, $contact);
                            }
    ...
    public function staff_reminders()
    {
        ...
        $this->db->where(db_prefix() . 'reminders.deleted_at IS NULL');
    ...
    public function get_table_data($where = [], $table)
    {
        $this->db->where($where);
        return $this->db->get($table)->result_array();
    }

    public function update_table_data($data, $id, $table)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . $table, $data);
        if ($this->db->affected_rows() > 0) {
            log_activity("$table updated by cron job [ID: $id, Updated data: " . json_encode($data) . "]");
            return true;
        }
        return false;
    }

    public function delete_temp_lead_inquiry_forms()
    {
