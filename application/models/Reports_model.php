...
class Reports_model extends App_Model
{
     ...
     */
    public function leads_monthly_report($param)
    {
        $result      = $this->db->query('select last_status_change from ' . db_prefix() . 'leads where MONTH(last_status_change) = ' . $param['month'] . ' AND YEAR(last_status_change) = ' . $param['year'] . ' AND status = 1 and lost = 0')->result_array();
        $month_dates = getAllDatesOfMonth($param['year'], $param['month']);
        ...
        foreach ($result as $lead) {
            ...
            foreach ($chart['labels'] as $date) {
                if (date('Y-m-d', strtotime($lead['last_status_change'])) == $date) {
    ...
    'datasets' => [
        [
            ...
            'backgroundColor' => 'rgba(37,155,35,0.6)',
            'borderColor'     => '#84c529',
            'borderWidth'     => 1,
            'data'            => $total_income,
            'yAxisID'         => 'yIncome',
     ...
     * @return array  chart data
     */
    public function leads_week_report($startdare, $enddate)
    {
        $this->db->where('CAST(last_status_change as DATE) >= "' . $startdare . '" AND CAST(last_status_change as DATE) <= "' . $enddate . '" AND status = 1 and lost = 0');
     ...
     */
    public function leads_sources_report($data)
    {
        ...
        foreach ($sources as $source) {
            ...
            array_push($chart['datasets'][0]['data'], total_rows(db_prefix() . 'leads', [
                'source' => $source['id'],
                'YEAR(last_status_change)'  => $data['year'],
                'MONTH(last_status_change)'  => $data['month'],
                'status' => 1,
                'lost'   => 0,
            ]));
        }
