...
function init_head($aside = true)
{
    ...
    //$CI->load->view('admin/settings/includes/tatatel_js');
    if ($aside == true) {
...
function is_manager()
{
    ...
    $is_manager = (isset($getStaff->role) && $getStaff->role == "3") ? true : false;
    if ($is_manager) {
        return true;
    }
    return false;
...
function render_admin_js_variables()
{
    ...
    echo 'var auto_logout_minutes = "' . get_option('auto_logout_minutes') . '";';

    $staffData = get_staff(get_staff_user_id());
    if (isset($staffData->notification_sound)) {
        if ($staffData->notification_sound == "1") {
            echo 'var notification_sound = "1";';
        } else {
            echo 'var notification_sound = "0";';
        }
    } else {
        echo 'var notification_sound = "0";';
    }
    ...
    echo 'app.available_tags = ' . json_encode(get_tags_unique(true)) . ';';
