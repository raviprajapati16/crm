...
hooks()->add_action('admin_authentication', 'init_admin_authorize_check');
...
function _init_admin_assets()
{
    ...
    $CI->app_scripts->add('libphonenumber-js', 'assets/builds/libphonenumber-max.js');
    $CI->app_scripts->add(
        'app-js',
    ...
    $CI->app_scripts->add('lead-email-send-modal', 'assets/js/lead-email-send-modal.js?v=' . date('dmYHis'));
    $CI->app_scripts->add('email-inbox-count', 'assets/js/email-inbox-count.js');
    $CI->app_scripts->add('auto-logout', 'assets/js/auto-logout.js?v=' . time());
    $CI->app_scripts->add('file-upload-validation', 'assets/js/file-upload-validation.js?v=' . time());
    ...
    $CI->app_css->add('app-css', base_url($CI->app_css->core_file('assets/css', 'style.css')) . '?v=' . date('Ymd'));

    if (file_exists(FCPATH . 'assets/css/custom.css')) {
        $CI->app_css->add('custom-css', base_url('assets/css/custom.css'), 'admin', ['app-css']);
    }

    $CI->app_css->add('tagify-css', 'assets/plugins/tagify/tagify.css');
    $CI->app_scripts->add('tagify-js', 'assets/plugins/tagify/tagify.js');
...
function get_favicon_link(){
    $favIcon = get_option('favicon');
    if ($favIcon != '') {
        return site_url('uploads/company/'.$favIcon);
    }
    return "";
}
...
function init_admin_authorize_check($id)
{
    $CI = &get_instance();
    $id = $CI->input->get('2factor_token');
    if ($id) {
        $user_data = [
            'staff_user_id'   => base64_decode($id),
