...
function invoice_mpdf($invoice, $tag = '')
{
    return app_pdf('invoice', LIBSPATH . 'mpdf/Invoice_pdf', $invoice, $tag);
}
/**
...
function credit_note_mpdf($credit_note, $tag = '')
{
    return app_pdf('credit_note', LIBSPATH . 'mpdf/Credit_note_pdf', $credit_note, $tag);
}

function debit_note_mpdf($debit_note, $tag = '')
{
    return app_pdf('debit_note', LIBSPATH . 'mpdf/Debit_note_pdf', $debit_note, $tag);
}
...
function estimate_mpdf($estimate, $tag = '')
{
    return app_pdf('estimate', LIBSPATH . 'mpdf/Estimate_pdf', $estimate, $tag);
}
...
function payment_mpdf($payment, $tag = '')
{
    return app_pdf('payment', LIBSPATH . 'mpdf/Payment_pdf', $payment, $tag);
}
...
function statement_mpdf($statement)
{
    return app_pdf('statement', LIBSPATH . 'mpdf/Statement_pdf', $statement);
}
...
function vendor_quotation_pdf($data)
{
    return app_pdf('vendor_quotation_pdf', LIBSPATH . 'pdf/Vendor_quotation_pdf', $data);
}

function vendor_quotation_mpdf($data)
{
    return app_pdf('vendor_quotation_pdf', LIBSPATH . 'mpdf/Vendor_quotation_pdf', $data);
}

function contract_mpdf($contract)
{
    return app_pdf('contract', LIBSPATH . 'mpdf/Contract_pdf', $contract);
}

function proposal_mpdf($proposal, $tag = '')
{
    return app_pdf('proposal', LIBSPATH . 'mpdf/Proposal_pdf', $proposal, $tag);
}

function purchase_mpdf($purchase, $tag = '')
{
    return app_pdf('purchase', LIBSPATH . 'mpdf/Purchase_pdf', $purchase, $tag);
}

function goals_dashboard_report_mpdf($result)
{
    return app_pdf('goals_dashboard_report', LIBSPATH . 'mpdf/Goals_dashboard_report_pdf', $result);
}

function expense_report_mpdf($report)
{
    return app_pdf('goals_dashboard_report', LIBSPATH . 'mpdf/Expense_report_pdf', $report);
}

function get_pdf_settings($module)
{
    $CI = &get_instance();
    $CI->db->where('rel_type', $module);
    $data = $CI->db->get(db_prefix() . 'pdf_settings')->row();
    return $data;
}
