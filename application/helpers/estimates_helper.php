...
{
    $CI = &get_instance();
    $CI->db->select('id')->from(db_prefix() . 'estimates')->where('deleted_at IS NULL')->order_by('id', 'desc')->limit(1);
    $query            = $CI->db->get();
...
function get_estimates_where_sql_for_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
        if (manager_employee_data_access_permission_check("estimates")) {
            $whereUser = '( ' . db_prefix() . 'estimates.addedfrom IN (' . get_manager_assigned_staff_ids('', true) . ')';
            if ($allow_staff_view_estimates_assigned == 1) {
                $whereUser .= ' OR sale_agent IN (' . get_manager_assigned_staff_ids('', true) . ')';
            }
            $whereUser .= ')';
        } else {
            $whereUser = '( ' . db_prefix() . 'estimates.addedfrom=' . $staff_id;
            if ($allow_staff_view_estimates_assigned == 1) {
...
function user_can_view_estimate($id, $staff_id = false)
{
    ...
    if ((has_permission('estimates', $staff_id, 'view_own') && $estimate->addedfrom == $staff_id)
        ...
        || estimate_manager_permission_check($id)
    ) {
        return true;
    }
...
function estimate_manager_permission_check($estimate_id)
{
    $CI = &get_instance();
    $CI->load->model('estimates_model');
    ...
    if (manager_employee_data_access_permission_check("estimates")) {
        ...
        if (in_array($estimate->sale_agent, $managerStaffIds) || in_array($estimate->addedfrom, $managerStaffIds) || $estimate->sale_agent == get_staff_user_id() || $estimate->addedfrom == get_staff_user_id()) {
            return true;
        }
    }
    return false;
