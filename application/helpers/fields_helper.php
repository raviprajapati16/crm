...
function all_type_input_render($fieldArray, $classNameCol = 'col-md-12', $requiredDisable = false)
{
    $requiredShow = $fieldArray['is_required'];
    if (!$requiredDisable) {
        $fieldArray['is_required'] = false;
        //unset($fieldArray['is_required']);
    }
    $selected_value = [];
    if (isset($fieldArray['selected_value'])) {
        $selected_value = array_values(array_filter(explode(",", $fieldArray['selected_value'])));
    }

    $html = "";
    $type = $fieldArray['type'];
    $type_options = [];
    if(isset($fieldArray['type_options'])){
        $type_options = array_values(array_filter(explode(",", $fieldArray['type_options'])));
    }
    $html .= '<div class="' . $classNameCol . '">';
    $html .= '<div class="form-group" data-type="' . $type . '" data-name="' . $fieldArray['name'] . '" data-required="' . ($fieldArray['is_required'] ? true : false) . '">';
    $html .= '<div class="control-label">' . ucfirst($fieldArray['label']) .  ($requiredShow ? ' <span class="text-danger">* </span> ' : '') . '</div>';
    if ($type == 'link' || $type == 'text' || $type == 'number' || $type == 'email') {
        $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ((isset($fieldArray['is_readonly']) && $fieldArray['is_readonly']) ? ' readonly="true"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . (isset($fieldArray['placeholder']) ? ' placeholder="' . $fieldArray['placeholder'] . '"' : '') . ' type="' . $type . '" name="' . $fieldArray['name'] . '" id="field' . $fieldArray['id'] . '" class="form-control' . (isset($fieldArray['className']) ? ' ' . $fieldArray['className'] : '') . '" value="' . (isset($fieldArray['selected_value']) ? $fieldArray['selected_value'] : '') . '"' . (isset($fieldArray['maxlength']) ? ' maxlength="' . $fieldArray['maxlength'] . '"' : '') . '>';
    } elseif ($type == 'colorpicker') {
        $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . (isset($fieldArray['placeholder']) ? ' placeholder="' . $fieldArray['placeholder'] . '"' : '') . ' type="color" name="' . $fieldArray['name'] . '" id="field' . $fieldArray['id'] . '" class="' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" value="' . (isset($fieldArray['selected_value']) ? $fieldArray['selected_value'] : '') . '"' . ($type == 'file' ? ' accept="' . get_form_accepted_mimes() . '" filesize="' . file_upload_max_size() . '"' : '') . '>';
    } elseif ($type == 'textarea') {
        $html .= '<textarea' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' id="field' . $fieldArray['id'] . '" name="' . $fieldArray['name'] . '" rows="' . (isset($fieldArray['rows']) ? $fieldArray['rows'] : '4') . '" class="form-control ' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" placeholder="' . (isset($fieldArray['placeholder']) ? $fieldArray['placeholder'] : '') . '">' . (isset($fieldArray['selected_value']) ? $fieldArray['selected_value'] : '') . '</textarea>';
    } elseif ($type == 'date_picker') {
        $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' placeholder="' . (isset($fieldArray['placeholder']) ? $fieldArray['placeholder'] : '') . '" type="text" class="form-control ' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . 'render-input-disabled datepicker" name="' . $fieldArray['name'] . '" id="field' . $fieldArray['id'] . '" value="' . (isset($fieldArray['selected_value']) ? $fieldArray['selected_value'] : '') . '" autocomplete="off">';
    }
    elseif ($type == 'timepicker') {
        $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' placeholder="' . (isset($fieldArray['placeholder']) ? $fieldArray['placeholder'] : '') . '" type="time" class="form-control ' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . 'render-input-disabled" name="' . $fieldArray['name'] . '" id="field' . $fieldArray['id'] . '" value="' . (isset($fieldArray['selected_value']) ? $fieldArray['selected_value'] : '') . '" autocomplete="off">';
    }
    elseif ($type == 'date_picker_time') {
        ...
        $html .= '<select' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . '' . (($type == 'multiselect') ? ' multiple="true"' : '') . ' class="selectpicker ' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" name="' . $fieldArray['name'] . ($type == 'multiselect' ? '[]' : '') . '" id="field' . $fieldArray['id'] . '"' . (isset($type_options) && count($type_options) > 10 ? 'data-live-search="true"' : '') . 'data-none-selected-text="' . (isset($fieldArray['placeholder']) ? $fieldArray['placeholder'] : 'Select Option') . '" data-width="100%">';
        $html .= '<option value="">Select option</option>';
        if (isset($type_options) && count($type_options) > 0) {
            foreach ($type_options as $option) {
                $html .= '<option value="' . $option . '" ' . (in_array($option, $selected_value) ? ' selected' : '') . '>' . ucfirst($option) . '</option>';
            }
        }
        $html .= '</select>';
        ...
        if (isset($type_options) && count($type_options) > 0) {
            ...
            foreach ($type_options as $checkbox) {
                $html .= '<label class="checkbox-inline" for="chk_' . $fieldArray['id'] . '_' . $i . '">';
                $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' class="' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" type="checkbox" id="chk_' . $fieldArray['id'] . '_' . $i . '" value="' . $checkbox . '" name="' . $fieldArray['name'] . '[]"' . (in_array($checkbox, $selected_value) ? ' checked' : '') . '>';
        ...
        if (isset($type_options) && count($type_options) > 0) {
            ...
            foreach ($type_options as $radio) {
                $html .= '<label class="radio-inline" for="radio_' . $fieldArray['id'] . '_' . $i . '">';
                $html .= '<input' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' class="' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" type="radio" id="radio_' . $fieldArray['id'] . '_' . $i . '" value="' . $radio . '" name="' . $fieldArray['name'] . '"' . (in_array($radio, $selected_value) ? ' checked' : '') . '>';
                $html .= ucfirst($radio);
                $html .= '</label>';
                $i++;
            }
        ...
        $html .= '<input ' . (isset($fieldArray['form']) ? ' form="' . $fieldArray['form'] . '"' : '') . ($fieldArray['is_required'] ? ' required="true"' : '') . ' type="file" class="form-control ' . (isset($fieldArray['className']) ? $fieldArray['className'] : '') . '" id="fileupload_' . $fieldArray['id'] . '" name="' . $fieldArray['name'] . '" />';
        if (isset($fieldArray['selected_value']) && !empty($fieldArray['selected_value'])) {
            if (isset($fieldArray['preview_url'])) {
                $html .= "<span class = 'file-preview-section'><i class='fa fa-paperclip' aria-hidden='true'></i> <a href='" . $fieldArray['preview_url'] . "' class='preview-file' target='_blank'>" . $fieldArray['selected_value'] . "</a> ".((isset($fieldArray['file_delete_class']) && !empty($fieldArray['file_delete_class'])) ?  "<i class='fa fa-trash ".$fieldArray['file_delete_class']." text-danger' aria-hidden='true'></i>" : '')."</span>";
            }
        }
    }

    if(isset($fieldArray['instructions']) && !empty($fieldArray['instructions'])){
        $html .= $fieldArray['instructions'];
    }
