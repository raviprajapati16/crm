...
{
    $logoIndex = ['logo', 'logo_dark', 'logo_white'];
    $success   = false;
...
function handle_company_brochure_upload()
{
    $success   = false;
    if (isset($_FILES['company_brochure']) && _perfex_upload_error($_FILES['company_brochure']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['company_brochure']['error']));
        return false;
    }
    if (isset($_FILES['company_brochure']['name']) && $_FILES['company_brochure']['name'] != '') {
        $path = get_upload_path_by_type('company');
        $tmpFilePath = $_FILES['company_brochure']['tmp_name'];
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            $extension          = strtolower(pathinfo($_FILES['company_brochure']['name'], PATHINFO_EXTENSION));
            $allowed_extensions = ['pdf','PDF'];
            if (!in_array($extension, $allowed_extensions)) {
                set_alert('warning', 'Image extension not allowed.');
                return false;
            }
            $filename    =  get_option('companyname').' - Brochure.' . $extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                update_option('company_brochure', $filename);
                $success = true;
            }
        }
    }
