...
if (!function_exists('format_customer_info')) {
    ...
    function format_customer_info($data, $for, $type, $companyLink = false)
    {
        ...
        } elseif ($for == 'debit_note') {
            $clientId = $data->vendorid;
        } elseif ($type == 'billing') {
        ...
        if ($for == 'statement') {
            ...
            $lead = get_lead($clientId);
            if (!empty($lead)) {
                $companyName = $lead->name;
            } else {
                $companyName = $data->deleted_vendor_name;
            }
        } elseif ($type == 'billing') {
        ...
        if ($for != "debit_note" && $companyLink && (!isset($data->deleted_customer_name) || (isset($data->deleted_customer_name) && empty($data->deleted_customer_name)))) {
...
if (!function_exists('format_proposal_info')) {
    ...
    function format_proposal_info($proposal, $for = '')
    {
        if (is_numeric($proposal)) {
            ...
            $proposal = $CI->proposals_model->get($proposal);
        }
        ...
        $format = _info_format_replace('gst_in', $gst_in, $format);
...
if (!function_exists('format_organization_info')) {
     ...
     * @return string
     */
    function format_organization_info($is_to = false)
    {
...
function get_items_by_type($type, $id, $where = [])
{
    $CI = &get_instance();
    $CI->db->select('items.*, main_group.name as main_group_name, sub_group.name as sub_group_name');
    $CI->db->from(db_prefix() . 'itemable as items');
    $CI->db->join(db_prefix() . 'items_groups as main_group', 'main_group.id = items.main_group_id', 'LEFT');
    $CI->db->join(db_prefix() . 'sub_groups as sub_group', 'sub_group.id = items.sub_group_id', 'LEFT');
    $CI->db->where('items.rel_id', $id);
    $CI->db->where('items.rel_type', $type);
    if (!empty($where)) {
        $CI->db->where($where);
    }
    $CI->db->order_by('items.item_order', 'asc');
...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    if (isset($item['main_group_id'])) {
        $data['main_group_id'] = $item['main_group_id'];
    }

    if (isset($item['sub_group_id'])) {
        $data['sub_group_id'] = $item['sub_group_id'];
    }
    ...
    if (isset($item['kind_of_packages'])) {
        $data['kind_of_packages'] = $item['kind_of_packages'];
    }

    if (isset($item['net_weight'])) {
        $data['net_weight'] = $item['net_weight'];
    }

    if (isset($item['gross_weight'])) {
        $data['gross_weight'] = $item['gross_weight'];
    }

    $CI->db->insert(db_prefix() . 'itemable', $data);
...
function update_sales_item_post($item_id, $data, $field = '')
{
    ...
    if ($field !== '') {
        ...
        if (isset($data['main_group_id'])) {
            $update['main_group_id'] = $data['main_group_id'];
        }
        if (isset($data['sub_group_id'])) {
            $update['sub_group_id'] = $data['sub_group_id'];
        }
        if (isset($data['item_id'])) {
            $update['item_id'] = $data['item_id'];
        }
        if (isset($data['kind_of_packages'])) {
            $update['kind_of_packages'] = $data['kind_of_packages'];
        }
        if (isset($data['net_weight'])) {
            $update['net_weight'] = $data['net_weight'];
        }
        if (isset($data['gross_weight'])) {
            $update['gross_weight'] = $data['gross_weight'];
        }
    }
...
function is_invoice_packlist_completed($id)
{
    $is_valid = true;
    $CI = &get_instance();

    // Check if all items have kind of packages, net weight, and gross weight
    $CI->db->select('kind_of_packages,net_weight,gross_weight');
    ...
    // Check invoice for total net weight, total gross weight, total packages
    $CI->db->select('total_packages,total_net_weight,total_gross_weight');
    $CI->db->where('id', $id);
    ...
    if (
        empty($invoice) ||
        $invoice->total_packages == 0 ||
        $invoice->total_net_weight == 0 ||
        $invoice->total_gross_weight == 0
    ) {
        $is_valid = false;
    }
...
function get_item_available_stock($item_id)
{
    $CI = &get_instance();
    ...
    $item_id = (int)$item_id;
    $year_condition_purchase = '';
    $year_condition_invoice = '';
    if (!empty($year) && is_numeric($year)) {
        $year = (int)$year;
        $year_condition_purchase = " AND YEAR(p.date) = {$year} ";
    ...
    $sql = "
        ...
        LEFT JOIN tblpurchase p ON p.id = ti.rel_id AND ti.rel_type = 'purchase'
    ...
    if ($query && $query->num_rows() > 0) {
        ...
        return (int)$result['available_qty'];
    }
