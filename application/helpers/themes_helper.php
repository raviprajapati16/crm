...
function add_default_theme_menu_items()
{
    ...
    if (!is_client_logged_in()) {
        ...
        if (has_contact_permission('assigned_Leads')) {
            add_theme_menu_item('assigned_Leads', [
                'name'     => "Assigned Leads",
                'href'     => site_url('clients/assigned_Leads'),
                'position' => 30,
            ]);
        }
        if (total_rows(db_prefix().'customer_media', ['customer_id' => get_client_user_id()]) > 0) {
            add_theme_menu_item('media', [
                'name'     => _l('Media'),
                'href'     => site_url('clients/media'),
                'position' => 30,
            ]);
        }
