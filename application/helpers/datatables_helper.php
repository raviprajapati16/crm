...
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
{
    ...
    if ($CI->input->post('order')) {
        $sOrder = 'ORDER BY ';
        if ($sTable == db_prefix() . 'proposals') {
            $sOrder .= ' FIELD(download_request, "2") DESC, ';
        }
        if ($sTable == db_prefix() . 'reminders') {
            $sOrder .= ' id DESC, ';
        }
        if ($sTable == db_prefix() . 'brochure') {
            $sOrder .= ' title ASC, ';
        }
        if ($sTable == db_prefix() . 'product_presentation') {
            $sOrder .= ' title ASC, ';
        }
         if ($sTable == db_prefix() . 'invoicepaymentrecords') {
            $sOrder .= ' date DESC, ';
        }
    ...
    if ($sTable == 'tblleads') {
        //$sOrder = "ORDER BY lapselead DESC, reminderdate asc, dateadded desc";
    }
