...
function get_relation_data($type, $rel_id = '')
{
    ...
    } elseif ($type == 'vendor' || $type == 'vendors') {
        if ($rel_id != '') {
            $CI->load->model('leads_model');
            $data = $CI->leads_model->get($rel_id);
        } else {
    ...
    elseif ($type == 'contact_book') {
        if ($rel_id != '') {
            $CI->load->model('contact_book_model');
            $data = $CI->contact_book_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_contact_book($q);
            $data = $search['result'];
        }
    }
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
    ...
    } elseif ($type == 'lead' || $type == 'leads' || $type == 'vendor' || $type == 'vendors') {
        if (is_array($relation)) {
            $id   = $relation['id'];
            $name = $relation['company'].' ('.$relation['name'].')';
            // if ($relation['email'] != '') {
            //     $name .= ' - ' . $relation['email'];
            // }
        } else {
        ...
        if (is_array($relation)) {
            ...
            $addedfrom  = isset($relation['created_by']) ? $relation['created_by'] : 0; 
            // if ($relation['email'] != '') {
            //     $name .= ' - ' . $relation['email'];
            // }
        } else {
            ...
            $name = $relation->firstname . ' ' . $relation->lastname;
            $name =  $relation->company.' ('.$name.')';
             $addedfrom  = isset($relation->created_by) ? $relation->created_by : 0; // <-- set addedfrom = created_by
            // if ($relation->email != '') {
            //     $name .= ' - ' . $relation->email;
            // }
        }
        $link = admin_url('contact_book');
    }
...
function init_relation_options($data, $type, $rel_id = '')
{
    ...
    $has_permission_contact_book_view = has_permission('contact_book', '', 'view');
    $is_admin                      = is_admin();
    $CI                            = &get_instance();
    $CI->load->model('projects_model');
    ...
    foreach ($data as $relation) {
        ...
        if ($type == 'project') {
            ...
            if (!has_permission('leads', '', 'view') && !leads_permission_allow_to_manager($relation['id'])) {
            ...
            if (!has_permission('vendors', '', 'view') && !leads_permission_allow_to_manager($relation['id'])) {
                if ($relation['assigned'] != get_staff_user_id() && $relation['addedfrom'] != get_staff_user_id() && $relation['is_public'] != 1 && $rel_id != $relation_values['id']) {
                    continue;
                }
            }
        } elseif ($type == 'customer') {
            if (!$has_permission_customers_view && !have_assigned_customers_new() && $rel_id != $relation_values['id']) {
                continue;
            } elseif (have_assigned_customers_new() && $rel_id != $relation_values['id'] && !$has_permission_customers_view) {
                if (!is_customer_admin_new($relation_values['id'])) {
                    continue;
                }
            }
            ...
            if (!$has_permission_contact_book_view && $rel_id != $relation_values['id'] && $relation_values['addedfrom'] != get_staff_user_id()) {
                continue;
            }
        }
