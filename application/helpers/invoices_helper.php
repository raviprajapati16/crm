...
function format_invoice_status($status, $classes = '', $label = true)
{
    ...
    if ($label == true) {
        return '<span class="bg-white label label-' . $label_class . ' ' . $classes . ' s-status invoice-status-' . $id . '">' . $status . '</span>';
    }
...
{
    $CI = &get_instance();
    $CI->db->select('id')->from(db_prefix() . 'invoices')->where('deleted_at IS NULL')->order_by('id', 'desc')->limit(1);
    $query           = $CI->db->get();
...
{
    $CI = &get_instance();
    $CI->db->select('number,prefix')->from(db_prefix() . 'invoices')->where('id', $id);
...
function invoice_number_prefix()
{
...
function get_invoices_percent_by_status($status)
{
    ...
    $total_invoices      = total_rows(db_prefix() . 'invoices', 'deleted_at IS NULL AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));

    $data            = [];
    $total_by_status = 0;
    if (!is_numeric($status)) {
        if ($status == 'not_sent') {
            $total_by_status = total_rows(db_prefix() . 'invoices', 'deleted_at IS NULL AND sent=0 AND status NOT IN(2,5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
        }
    } else {
        $total_by_status = total_rows(db_prefix() . 'invoices', 'status = ' . $status . ' AND deleted_at IS NULL AND status NOT IN(5)' . (!$has_permission_view ? ' AND (' . get_invoices_where_sql_for_staff(get_staff_user_id()) . ')' : ''));
    }
...
function staff_has_assigned_invoices($staff_id = '')
{
    ...
    if (is_numeric($cache)) {
        ...
        $result = total_rows(db_prefix() . 'invoices', ['sale_agent' => $staff_id, 'deleted_at' => NULL]);
...
function get_invoices_where_sql_for_staff($staff_id)
{
    ...
    if ($has_permission_view_own) {
        if (manager_employee_data_access_permission_check("invoices")) {
            $whereUser = '( ' . db_prefix() . 'invoices.addedfrom IN (' . get_manager_assigned_staff_ids('', true) . ')';
            if ($allow_staff_view_invoices_assigned == 1) {
                $whereUser .= ' OR sale_agent IN (' . get_manager_assigned_staff_ids('', true) . ')';
            }
            $whereUser .= ')';
        } else {
            $whereUser = '( ' . db_prefix() . 'invoices.addedfrom=' . $staff_id;
            if ($allow_staff_view_invoices_assigned == 1) {
...
function user_can_view_invoice($id, $staff_id = false)
{
    ...
    if ((has_permission('invoices', $staff_id, 'view_own') && $invoice->addedfrom == $staff_id)
        || ($invoice->sale_agent == $staff_id && get_option('allow_staff_view_invoices_assigned') == '1'
            || invoice_manager_permission_check($id)
        )
    ) {
        return true;
    }
...
// old payment records update proposal id
function payment_update_old_records()
{
    $CI = &get_instance();
    $result = [];
    $paymentData = $CI->db->get(db_prefix() . 'invoicepaymentrecords')->result_array();
    if (!empty($paymentData)) {
        foreach ($paymentData as $key => $item) {
            if (empty($item['proposal_id'])) {
                if (!empty($item['invoiceid']) && $item['invoiceid'] != 0) {
                    $CI->db->where('invoice_id', $item['invoiceid']);
                    $proposalData = $CI->db->get(db_prefix() . 'proposals')->row();
                    if (!empty($proposalData)) {
                        $CI->db->where('id', $item['id']);
                        $CI->db->update(db_prefix() . 'invoicepaymentrecords', ["proposal_id" => $proposalData->id]);
                        if ($CI->db->affected_rows() > 0) {
                            $result['invoice_based_updated_ids'][] = $item['id'];
                        }
                    } else {
                        $CI->db->where('invoiceid', $item['invoiceid']);
                        $CI->db->where('deleted_at IS NULL');
                        $estimateData = $CI->db->get(db_prefix() . 'estimates')->row();
                        if (!empty($estimateData)) {
                            $CI->db->where('estimate_id', $estimateData->id);
                            $proposalData = $CI->db->get(db_prefix() . 'proposals')->row();
                            if (!empty($proposalData)) {
                                $CI->db->where('id', $item['id']);
                                $CI->db->update(db_prefix() . 'invoicepaymentrecords', ["proposal_id" => $proposalData->id]);
                                if ($CI->db->affected_rows() > 0) {
                                    $result['estimate_based_updated_ids'][] = $item['id'];
                                }
                            }
                        }
                    }
...
function update_contract_terms($paymentid = "", $invoice_id = "", $proposal_id = "")
{
    $CI = &get_instance();
    if (!empty($paymentid)) {
        $CI->db->where('id', $paymentid);
        $paymentTerms = $CI->db->get(db_prefix() . 'invoicepaymentrecords')->row();
        if (!empty($paymentTerms)) {
            if (!empty($paymentTerms->proposal_id)) {
                $proposal_id = $paymentTerms->proposal_id;
            } else {
                $proposal_id = get_proposal_id_by_invoice($paymentTerms->invoiceid);
            }
        }
    } else if (!empty($invoice_id)) {
        $proposal_id = get_proposal_id_by_invoice($invoice_id);
    }

    if (!empty($proposal_id)) {
        $CI->db->select('contract_id');
        $CI->db->where('proposal_id', $proposal_id);
        $contracts = $CI->db->get(db_prefix() . 'contract_proposals')->result_array();
        if (!empty($contracts)) {
            $contracts_ids = array_column($contracts, 'contract_id');
            foreach ($contracts_ids as $contract_id) {
                update_payment_terms_status($contract_id);
            }
        }
    }
}

function invoice_manager_permission_check($invoice_id)
{
    $CI = &get_instance();
    $CI->load->model('invoices_model');
    ...
    if (manager_employee_data_access_permission_check("invoices")) {
        ...
        if (in_array($invoice->sale_agent, $managerStaffIds) || in_array($invoice->addedfrom, $managerStaffIds) || $invoice->sale_agent == get_staff_user_id() || $invoice->addedfrom == get_staff_user_id()) {
            return true;
        }
    }
    return false;
