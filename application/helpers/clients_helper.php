...
function app_init_customer_profile_tabs()
{
    ...
    $CI->app_tabs->add_customer_profile_tab('customer_media', [
        'name' => _l('customer_media'),
        'icon' => 'fa fa-files-o',
        'view' => 'admin/clients/groups/customer_media',
        'visible' => (has_permission('customer_media', '', 'view')),
        'position' => 95,
    ]);

    $CI->app_tabs->add_customer_profile_tab('customer_assigned_leads', [
        'name' => "Assigned Leads",
        'icon' => 'fa fa-tty',
        'view' => 'admin/clients/groups/customer_assigned_leads',
        'position' => 100,
    ]);
...
function get_client_id_by_contact_id($id)
{
    $CI = &get_instance();
    $CI->db->select('userid')->from(db_prefix() . 'contacts')->where('id', $id);
    return $CI->db->get()->row()->userid;
}
...
function is_primary_contact($contact_id = '')
{
    ...
    if (total_rows(db_prefix() . 'contacts', ['id' => $contact_id, 'is_primary' => 1, 'deleted_at' => NULL]) > 0) {
        return true;
    }
...
//for manager check
function is_customer_admin_new($id, $staff_id = '')
{
    $CI = &get_instance();

    if (empty($staff_id)) {
        if (manager_employee_data_access_permission_check("customers")) {
            $staff_id = get_manager_assigned_staff_ids();
        } else {
            $staff_id = is_numeric($staff_id) ? $staff_id : get_staff_user_id();
        }
    }
    $CI->db->from(db_prefix() . 'customer_admins');
    $CI->db->where('customer_id', $id);
    $CI->db->where('deleted_at IS NULL');
    if (is_array($staff_id)) {
        $CI->db->where_in('staff_id', $staff_id);
    } else {
...
function have_assigned_customers($staff_id = '')
{
    ...
    if (is_numeric($cache)) {
        ...
        if (manager_employee_data_access_permission_check("customers")) {
            $where = "deleted_at IS NULL AND staff_id IN (" . get_manager_assigned_staff_ids('', true) . ")";
            $result = total_rows(db_prefix() . 'customer_admins', $where);
        } else {
...
function have_assigned_customers_new($staff_id = '')
{
    $CI = &get_instance();
    if (empty($staff_id)) {
        if (is_manager()) {
...
function get_assigned_customers_ids_by_staff($staff_id = '')
{
    $CI = &get_instance();
    if (empty($staff_id)) {
        if (is_manager()) {
 ...
 * @return  array
 */
function get_all_customer_attachments($id, $real_type = "")
{
    $CI = &get_instance();
    ...
    if ($permission_contracts_view || $permission_contracts_own) {
        ...
        $CI->db->where('rel_type', $real_type);
    ...
    //Contract based on vendor
    $CI->db->select('is_vendor')->where('id', $customer->leadid);
    ...
    if (!empty($lead)) {
        ...
        if ($permission_contracts_view || $permission_contracts_own) {
            ...
            $CI->db->where('rel_type', 'vendor');
            $CI->db->where('client', $customer->leadid);
            if (!$permission_contracts_view) {
                $CI->db->where('addedfrom', get_staff_user_id());
            }
            $CI->db->from(db_prefix() . 'contracts');
    ...
    if ($has_permission_proposals_view || $has_permission_proposals_own || get_option('allow_staff_view_proposals_assigned') == 1) {
        ...
        if ($customer->leadid != null) {
            ...
            $CI->db->from(db_prefix() . 'proposals');
            $proposals_leads = $CI->db->get()->result_array();
            $proposals = array_merge($proposals, $proposals_leads);
        }
    ...
    $has_permission_projects_own = has_permission('projects', '', 'view_own');
    if ($has_permission_projects_view || $has_permission_projects_own) {
        $CI->db->select('clientid,id');
        $CI->db->where('clientid', $id);
        ...
        $projectIds = $ids;
        if (count($ids) > 0) {
            $CI->db->where_in('project_id', $ids);
            $_attachments = $CI->db->get(db_prefix() . 'project_files')->result_array();
            foreach ($_attachments as $_att) {
                $_att['attachment_key'] = "project_file_" . $_att['id'];
                $_att['rel_type'] = "project";
                array_push($attachments['project'], $_att);
            }
        }
    }
    ...
    if (!empty($expenseIds)) {
        ...
        $CI->db->where_in('rel_id', $expenseIds);
        $CI->db->where('rel_type', 'expense');
        $CI->db->group_end();
    }
    ...
    if (!empty($invoicesIds)) {
        $CI->db->or_group_start();
        $CI->db->where_in('rel_id', $invoicesIds);
        $CI->db->where('rel_type', 'invoice');
        $CI->db->group_end();
    }
    ...
    if (!empty($estimateIds)) {
        $CI->db->or_group_start();
        $CI->db->where_in('rel_id', $estimateIds);
        $CI->db->where('rel_type', 'estimate');
        $CI->db->group_end();
    }
    ...
    if (!empty($contractsIds)) {
        $CI->db->or_group_start();
        $CI->db->where_in('rel_id', $contractsIds);
        $CI->db->where('rel_type', 'contract');
        $CI->db->group_end();
    }
    ...
    if (!empty($proposalIds)) {
        $CI->db->or_group_start();
        $CI->db->where_in('rel_id', $proposalIds);
        $CI->db->where('rel_type', 'proposal');
        $CI->db->group_end();
    }
    ...
    if (!empty($projectIds)) {
        $CI->db->or_group_start();
        $CI->db->where_in('rel_id', $projectIds);
    ...
    if (!$has_permission_tasks_view && is_staff_logged_in()) {
        $CI->db->where($noPermissionQuery);
    }
...
{
    return 'CASE ' . db_prefix() . 'clients.company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM ' . db_prefix() . 'contacts WHERE userid = ' . db_prefix() . 'clients.userid and is_primary = 1) ELSE ' . db_prefix() . 'clients.company END as company';
}
...
function get_lead_customer_assign_status()
{
    return [
        0 => [
            'name'  => 'Not Started',
