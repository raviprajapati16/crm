...
function init_relation_tasks_table($table_attributes = [])
{
    ...
    if (has_permission('tasks', '', 'create')) {
        ...
        if ($table_attributes['data-new-rel-type'] == 'lead') {
            echo tutorialLinkButtonRender('lead-task-create-btn', 'right');
        }
    }
...
{
    if (manager_employee_data_access_permission_check("tasks")) {
        $_tasks_where = '(' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid IN (' . get_manager_assigned_staff_ids('', true) . ')) OR ' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE staffid IN (' . get_manager_assigned_staff_ids('', true) . ')) OR (addedfrom IN (' . get_manager_assigned_staff_ids('', true) . ') AND is_added_from_contact=0)';
        if (get_option('show_all_tasks_for_project_member') == 1) {
            $_tasks_where .= ' OR (' . db_prefix() . 'tasks.rel_type="project" AND ' . db_prefix() . 'tasks.rel_id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id IN (' . get_manager_assigned_staff_ids('', true) . ')))';
        }
