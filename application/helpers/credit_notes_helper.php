...
{
    $total = sum_from_table(db_prefix() . 'credits', ['field' => 'amount', 'where' => ['invoice_id' => $id, 'deleted_at' => NULL]]);
...
{
    $CI = & get_instance();
    $CI->db->select('id')->from(db_prefix() . 'creditnotes')->where('deleted_at IS NULL')->order_by('id', 'desc')->limit(1);
    $query            = $CI->db->get();
