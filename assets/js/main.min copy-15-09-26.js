...
$(function () {
    ...
    var LeadsServerParams = {
        ...
        "folloup": "[name='followup_date']",
        "countries": "[name='countries[]']",
        "states": "[name='states[]']",
        "cities": "[name='cities[]']",
        "datefilter": "[name='date_by']",
    ...
    if (table_leads.length) {
        ...
        $("#followup_date").on("change", function () {
            table_leads.DataTable().ajax.reload().columns.adjust().responsive.recalc();
        });

        $("#from_date").on("change", function () {
            var datefilter = $("#date_by").val();
            if (datefilter == '-1' || $("#date_by").val() == '') {
                $("#date_by").focus();
    ...
    $("body").on('change', '.f_client_id select[name="clientid"]', function () {
        ...
        requestGetJSON('invoices/client_change_data/' + val + '/' + currentInvoiceID).done(function (response) {
            ...
            if (typeof setInvoiceLocationValues === 'function' && $('select[data-location-group="invoice-shipping"][data-location-role="country"]').length) {
                setInvoiceLocationValues(
                    'invoice-shipping',
            ...
            if (typeof cacheInvoiceShippingField === 'function') {
                ...
                if (_sbs['shipping_notify_party']) cacheInvoiceShippingField('shipping_notify_party', _sbs['shipping_notify_party']);
            }
...
function init_lead_modal_data(id, url, isEdit) {
    ...
    if (module == "vendors") {
        requestURL += (requestURL.indexOf('?') > -1 ? '&' : '?') + 'vendor=true';
    }
    if (module == "meeting_dashboard") {
        requestURL += (requestURL.indexOf('?') > -1 ? '&' : '?') + 'pvd=true';
    }
        ...
        setTimeout(function () {
            if (nIds.length > 0) {
                $.each(nIds, function (i, notId) {
                    ...
                    alert_float('info', $not.find('.notification-title').text(), 5000);
                    if (notification_sound == "1") { var audio = new Audio(site_url + 'assets/notification_sound_new.wav'); audio.muted = false; audio.play(); }
                    $.notify("", {
