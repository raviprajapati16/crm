                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2 pull-left">
                                <select name="contract_status[]" class="selectpicker" multiple="true" id="contract_status" data-width="100%" data-none-selected-text="Filter By Contract status">
                                    <option value=""></option>
                            ...
                            <div class="col-md-2 pull-left">
                                <select name="manager_filter" class="selectpicker" id="manager_filter" data-width="100%" data-none-selected-text="Filter By Manager">
                                    <option value=""></option>
                                    <?php
                                    foreach ($staff_members as $staffmember) { ?>
                            ...
                            <div class="col-md-2">
                                <div class="checkbox">
                                    ...
                                    <label for="exclude_inactive"><?php echo _l('exclude_inactive'); ?> <?php echo _l('staff'); ?></label>
                                </div>
                            </div>
                        </div>
...
<script>
    $(function() {
        ...
        var StaffServerParams = {
            ...
            "contract_status": "[name='contract_status[]']",
            "exclude_inactive": "[name='exclude_inactive']:checked",
            "manager_filter": "[name='manager_filter']",
        };
        table_staff = $('table.table-table_staff');
        // initDataTable(table_staff, admin_url + 'hrm/table', '', '', StaffServerParams);
        initDataTable(table_staff, admin_url + 'hrm/table', '', '', StaffServerParams).on('draw.dt', function () {
    var PageInfo = table_staff.DataTable().page.info();
    table_staff.DataTable().column(0, { page: 'current' }).nodes().each(function (cell, i) {
        ...
        $('#exclude_inactive').on('change', function() {
            table_staff.DataTable().ajax.reload()
                .columns.adjust()
                .responsive.recalc();
        });
