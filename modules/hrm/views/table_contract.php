...
array_push($filter, 'AND active = 1');
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'staff_contract.id_contract', 'firstname','lastname','type_name', 'allowance_val', 'form_name', 'salary_val', 'duration', 'unit']);
...
foreach ($rResult as $aRow) {
    ...
    $subjectOutput = '<a href="' . admin_url('hrm/contract/' . $aRow['id']) . '">' . $aRow['firstname'] .' '. $aRow['lastname'] . '</a>';
    ...
    if($aRow['contract_status'] == "Permanent"){
        $_data = "<span class='label label-success'>".$aRow['contract_status']."</span>";
    } else if($aRow['contract_status'] == "Probation" || $aRow['contract_status'] == "Bond"){
        $_data = "<span class='label label-warning'>".$aRow['contract_status']."</span>";
    }
    $row[] = $_data;
