...
Description: Human Resource Management module for CRM
Version: 2.3.0
...
function hrm_module_init_menu_items()
{
    ...
    $dashbordBadge = "";
    $hr_menu_badge = "";
    $expireCount = get_count_staff_expire_contract();
    if ($expireCount > 0) {
        $dashbordBadge = '<span class="badge menu-badge bg-warning">' . $expireCount . '</span>';
        $hr_menu_badge = ' <i class="fa fa-bell shake-animation text-warning" aria-hidden="true"></i>';
    }

    if (has_permission('hrm', '', 'view')) {
        $CI->app_menu->add_sidebar_menu_item('HRM', [
            'name'     => _l('hrm') . ' ' . $hr_menu_badge,
        ...
        $CI->app_menu->add_sidebar_children_item('HRM', [
            'slug'     => 'hrm_dashboard',
            'name'     => _l('dashboard') . ' ' . $dashbordBadge,
            'icon'     => 'fa fa-home',
            'href'     => admin_url('hrm'),
        ]);
        $CI->app_menu->add_sidebar_children_item('HRM', [
...
function hrm_add_footer_components()
{
    ...
    if (strpos($viewuri, 'hrm/member') !== false) {
