...
class hrm extends AdminController
{
    ...
    public function staff_infor()
    {
        ...
        $data['managers'] = $this->hrm_model->get_staff('', ['active' => 1]);
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if (checkDuplicateActiveStaffUser($data['email'])) {
                    set_alert('error', "User already exists and active with same email.");
                }
                ...
                if (checkDuplicateActiveStaffUser($data['email'], $id)) {
                    set_alert('error', "User already exists and active with same email.");
                }
                handle_staff_profile_image_upload($id);
    ...
    {
        if (!is_admin()) {
            access_denied("Only admin can delete staff account.");
        }
    ...
    public function contract($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $uniqueCheck = $this->hrm_model->contract_unique_check($data['staff']);
                if ($uniqueCheck == 0) {
                    ...
                    set_alert('warning', "Contract already exists for this staff user.");
                }
                ...
                $uniqueCheck = $this->hrm_model->contract_unique_check($data['staff'], $id);
                if ($uniqueCheck == 0) {
