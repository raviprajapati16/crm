...
function get_staff_in_dayoff($id)
{
    $CI = &get_instance();
    if ($id != '0'  && !empty($id)) {
        $data = $CI->db->query('SELECT CONCAT(firstname, " ", lastname) AS name FROM ' . db_prefix() . 'staff WHERE staffid IN (' . $id . ')')->result_array();
        if (!empty($data)) {
...
function get_hrm_department_ids_by_staffid($staff_id = '')
{
    if ($staff_id == '') {
        $staff_id = get_staff_user_id();
    }
    $CI = &get_instance();
    $arr_dept = $CI->db->query('select departmentid from '.db_prefix().'staff_departments where staffid = ' . $staff_id)->result_array();
    if (count($arr_dept) > 0) {
        return array_column($arr_dept,"departmentid");
    }
    return [];
}
