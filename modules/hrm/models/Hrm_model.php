...
class hrm_model extends App_Model
{
    ...
    public function check_email($email, $id = null)
    {
        ...
        $email = $this->db->get(db_prefix() . 'staff')->row_array();
    }
    ...
    public function delete_contract($id)
    {
        ...
        // $this->db->delete(db_prefix() . 'staff_contract_detail');
        $this->db->where('id_contract', $id);
        $this->db->update(db_prefix() . 'staff_contract', array("deleted_at" => date('Y-m-d H:i:s'), "deleted_by" => get_staff_full_name()));
        if ($this->db->affected_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function add_day_off($data)
    {
        ...
        foreach ($data['break_date'] as $key => $val) {
            $this->db->insert(db_prefix() . 'day_off', [
                ...
                'staff_id' => (isset($data['staff_id'][$key])) ? implode(",", $data['staff_id'][$key]) : NULL,
            ]);
            $insert_id = $this->db->insert_id();
            if ($insert_id) {
                $insert_success++;
    ...
    public function update_day_off($data, $id)
    {
        ...
        $this->db->update(db_prefix() . 'day_off', [
            ...
            'add_from' => get_staff_user_id(),
            'staff_id' => (isset($data['staff_id'])) ? implode(",", $data['staff_id']) : NULL,
        ]);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    public function get_data_staff_payroll($staffid, $column_key, $column_calculation, $month)
    {
        ...
        if ($monthly_KPI_points != '') {
            ...
            foreach ($res_monthly_KPI_points['monthly_KPI_points'] as $mkp) {
                $mark += $this->goals_model->calculate_goal_achievement_new($mkp['id'], $staffid)['percent'];
                $avg++;
            }
    ...
    public function get_hrm_dashboard_data()
    {
        ...
        $staff_birthday = $this->db->query('SELECT * FROM tblstaff WHERE MONTH(birthday) = ' . date('m') . ' ORDER BY birthday ASC')->result_array();
        ...
        $overdue_contract = $this->db->query('
            SELECT *
            FROM tblstaff_contract
            JOIN tblstaff ON tblstaff_contract.staff = tblstaff.staffid
            WHERE tblstaff.active = 1 AND tblstaff_contract.end_valid < "' . date('Y-m-d') . '"
        ')->result_array();

        $expire_contract = $this->db->query('
            SELECT *
            FROM tblstaff_contract
            JOIN tblstaff ON tblstaff_contract.staff = tblstaff.staffid
            WHERE tblstaff.active = 1
            AND tblstaff_contract.end_valid <= "' . date('Y-m-d', strtotime('+7 days', strtotime(date('Y-m-d')))) . '"
            AND tblstaff_contract.end_valid >= "' . date('Y-m-d') . '"
        ')->result_array();
        ...
        $data_hrm['staff_working_data'] = $staff_working;
        return $data_hrm;
    }
    ...
    public function contract_unique_check($staffid, $contractid = "")
    {
        ...
        $query = $this->db->get(db_prefix() . 'staff_contract');
