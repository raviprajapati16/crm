                ...
                foreach ($unfinished_surveys_send_log as $_survey) {
                    ...
                    foreach ($emails as $data) {
                        if (stripos($survey->description, '{survey_link}') !== false) {
                            $emailDescription = str_ireplace('{survey_link}', '<a href="' . site_url('survey/' . $survey->surveyid . '/' . $survey->hash.'/'.$data['id']) . '" target="_blank">' . $survey->subject . '</a>', $survey->description);
                        }
                        ...
                        $this->ci->db->update(db_prefix() . 'surveysemailsendcron', ["is_send" => 1]);
                    }
