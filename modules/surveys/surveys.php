...
function surveys_get_choice_question_answers_id($questionid, $surveyid, $result_set_id)
{
    $CI = &get_instance();
    $CI->db->select('*');
    $CI->db->from(db_prefix() . 'form_results');
    $CI->db->where('questionid', $questionid);
    $CI->db->where('rel_id', $surveyid);
    $CI->db->where('resultsetid', $result_set_id);
    $CI->db->where('rel_type', 'survey');

    return $CI->db->get()->result_array();
}

function surveys_get_choice_question_answers($questionboxdescriptionids)
{
    $CI = &get_instance();
    $CI->db->select('description');
    $CI->db->from(db_prefix() . 'form_question_box_description');
    $CI->db->where_in('questionboxdescriptionid', $questionboxdescriptionids);
    return $CI->db->get()->result_array();
}

function surveys_single_participate_details($result_set_id)
{
    $CI = &get_instance();
    $CI->db->select('surveysemailsendcron.*');
    $CI->db->from(db_prefix() . 'surveyresultsets');
    $CI->db->join(db_prefix() . 'surveysemailsendcron', db_prefix() . 'surveyresultsets.survey_cron_id = ' . db_prefix() . 'surveysemailsendcron.id', 'left');
    $CI->db->where(db_prefix() . 'surveyresultsets.resultsetid', $result_set_id);
    return $CI->db->get()->row_array();
}

function surveys_participate_users_details($surveyid)
{
    $CI = &get_instance();
    $CI->db->select('surveysemailsendcron.*,surveyresultsets.resultsetid');
    $CI->db->from(db_prefix() . 'surveyresultsets');
    $CI->db->join(db_prefix() . 'surveysemailsendcron', db_prefix() . 'surveyresultsets.survey_cron_id = ' . db_prefix() . 'surveysemailsendcron.id', 'left');
    $CI->db->where(db_prefix() . 'surveyresultsets.surveyid', $surveyid);
    return $CI->db->get()->result_array();
}