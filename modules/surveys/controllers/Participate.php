...
{
    public function index($id, $hash, $cron_log_id = NULL)
    {
        ...
        if ($post_data) {
            $post_data['survey_cron_id'] = $cron_log_id;
            $success = $this->surveys_model->add_survey_result($id, $post_data);
            if ($success) {
                $survey = $this->surveys_model->get($id);
                ...
                if (!empty($cron_log_id)) {
                    $redirectURL = site_url('survey/' . $id . '/' . $hash . '/' . $cron_log_id . '?participated=yes');
                } else {
                    $redirectURL = site_url('survey/' . $id . '/' . $hash . '?participated=yes');
                }

                redirect(hooks()->apply_filters('survey_default_redirect', $redirectURL));
            }
        }
